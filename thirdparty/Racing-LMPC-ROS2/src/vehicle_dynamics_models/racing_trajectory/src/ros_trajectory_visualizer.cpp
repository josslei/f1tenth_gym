...
void ROSTrajectoryVisualizer::change_trajectory(RacingTrajectory &trajectory) {
  ...
  const auto abscissa = casadi::DM::linspace(0.0, trajectory.total_length(),
                                             1001)(casadi::Slice(0, -1));
  const auto N = abscissa.size1();

  abscissa_polygon_msg_ = std::make_shared<PolygonStamped>();
  abscissa_polygon_msg_->header.frame_id = "map";
  const auto pts =
      casadi::DM::horzcat({trajectory.x_interpolation_function()(abscissa)[0],
  ...
  const auto &left_boundary_frenet =
      casadi::DM::horzcat(
          {abscissa,
           trajectory.left_boundary_interpolation_function()(abscissa)[0],
           casadi::DM::zeros(N)})
          .T();
  const auto &right_boundary_frenet =
      casadi::DM::horzcat(
          {abscissa,
...
void ROSTrajectoryVisualizer::attach_ros_publishers(rclcpp::Node *node,
                                                    const double &dt,
                                                    const bool &vis_boundary,
                                                    const bool &vis_abscissa) {
  node_ = node;
  if (vis_boundary) {
    left_boundary_polygon_pub_ =
        node->create_publisher<PolygonStamped>("left_boundary_polygon", 1);
    right_boundary_polygon_pub_ =
        node->create_publisher<PolygonStamped>("right_boundary_polygon", 1);
  }
  if (vis_abscissa) {
    abscissa_polygon_pub_ =
        node->create_publisher<PolygonStamped>("abscissa_polygon", 1);
  }
  ...
  static_vis_timer_ = node->create_wall_timer(
      ...
      std::bind(&ROSTrajectoryVisualizer::on_static_vis_timer, this),
      vis_callback_group_);
}
