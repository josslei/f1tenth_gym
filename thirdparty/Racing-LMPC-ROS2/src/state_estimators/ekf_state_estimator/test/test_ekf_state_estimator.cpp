...
const auto share_dir =
    ament_index_cpp::get_package_share_directory("ekf_state_estimator");
...
EKFStateEstimator::SharedPtr get_ekf() {
  ...
  options.arguments(
      {"--ros-args", "--params-file",
       ...
       share_dir + "/param/sample_ekf.param.yaml"});
  ...
  auto config =
      lmpc::state_estimator::ekf_state_estimator::load_parameters(&test_node);
...
TEST(EKFStateEstimatorTest, EKFStateEstimatorSolveTest) {
  ...
  const auto bound_left_intp =
      DM::interp1d(t_vec, bound_left.T(), t_intp.get_elements(), "", false).T();
  const auto bound_right_intp =
      DM::interp1d(t_vec, bound_right.T(), t_intp.get_elements(), "", false)
          .T();
  const auto X_optm_ref_intp =
      DM::interp1d(t_vec, X_optm_ref, t_intp.get_elements(), "", false).T();
  const auto U_optm_ref_intp =
      DM::interp1d(T_accum(Slice(0, -1)).get_elements(), U_optm_ref,
                   t_intp(Slice(0, -1)).get_elements(), "", false)
          .T();
  const auto T_optm_ref_intp = DM::zeros(N - 1) + t_intp(1) - t_intp(0);

  auto sol_in = casadi::DMDict{{"X_ref", X_optm_ref_intp},
