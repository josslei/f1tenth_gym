...
#define DCAST(m) static_cast<double>(m)
...
EKFStateEstimator::EKFStateEstimator(
    ...
    : config_(ekf_config), model_(model),
      ...
      initialized_(false), hs_(), h_jacs_(), x_(config_->x0),
      u_(casadi::DM::zeros(model_->nu(), 1)), P_(config_->P0),
      K_(model_->nx(), 0) {
  ...
  const auto xip1 =
      rk4_(casadi::SXDict{{"x", x}, {"u", u}, {"dt", dt}, {"k", 0.0}})
          .at("xip1");
  const auto F = casadi::SX::jacobian(xip1, x);
  F_ = casadi::Function("discrete_dynamics_jacobian", {x, u, dt}, {F},
                        {"x", "u", "dt"}, {"F"});
}
...
SingleTrackPlanarModel &EKFStateEstimator::get_model() { return *model_; }

const int64_t &EKFStateEstimator::get_latest_timestamp() const {
  return nanosec_;
}

const bool &EKFStateEstimator::is_initialized() const { return initialized_; }

void EKFStateEstimator::register_observation(const std::string &name,
                                             const casadi_int &nz,
                                             casadi::Function &h) {
  if (is_initialized()) {
...
void EKFStateEstimator::update_observation(const StrOpt &name,
  ...
  const auto in_dict =
      casadi::DMDict{{"x", x_}, {"u", u_}, {"k", 0.0}, {"dt", dt_ns * 1e-9}};
  ...
  if (name.has_value()) {
    ...
    auto R = in.at("R");
    if (!(check_nan_inf(z, "input observation z") &&
          check_nan_inf(R, "input observation covariance R"))) {
      // NaN and Inf check fails for this input. Carry out pure prediction.
      logger_.send_log(utils::LogLevel::WARN,
                       "NaN or Inf detected in filter input. Falling back to a "
      ...
      // update covariance
      P_ = DM::mtimes(
          DM::eye(model_->nx()) - DM::mtimes(K_(Slice(), slice_z), H), P_p);
  ...
  for (casadi_int i = 0; i < static_cast<casadi_int>(model_->nx()); i++) {
    x_(i) =
        std::clamp<double>(DCAST(x_(i)), config_->x_min[i], config_->x_max[i]);
  }
...
void EKFStateEstimator::update_control(const casadi::DM &u) { u_ = u; }

utils::Logger &EKFStateEstimator::get_logger() { return logger_; }

bool EKFStateEstimator::check_nan_inf(const casadi::DM &m,
                                      const std::string &name) {
  if (m.is_regular()) {
    return true;
  } else {
    const auto what =
        "NaN or Inf detected in matrix \"" + name + "\":\n" + m.get_str();
...
bool EKFStateEstimator::check_cov(casadi::DM &cov, const std::string &name) {
  ...
  if (has_issue) {
    const auto what = "Covariance matrix \"" + name +
                      "\" is ill-formed. It must be non-negative and have "
                      "positive diagnals:\n" +
                      cov.get_str();
...
const casadi::DM &EKFStateEstimator::get_latest_estimate() const { return x_; }
