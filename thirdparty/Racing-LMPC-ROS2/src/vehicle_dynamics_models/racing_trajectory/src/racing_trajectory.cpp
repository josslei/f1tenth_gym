...
RacingTrajectory::RacingTrajectory(const casadi::DM &traj)
...

  {
    ...
    // lets repeat the first 3 points at the end and the last 3 points at the
    // beginning. note that we are actually appending the first 4 points to the
    // end in order to close the trajectory loop.

    // append the first 4 points
    interpolants =
        DM::horzcat({interpolants, interpolants(Slice(), Slice(0, 4))});

    // make sure the distance measure is continuous
    interpolants(TrajectoryIndex::DIST_TO_SF_BWD,
                 Slice(-4, std::numeric_limits<casadi_int>::max())) +=
        total_length_;

    // prepend the last 3 points
    interpolants =
        DM::horzcat({interpolants(Slice(), Slice(-7, -4)), interpolants});
    ...
    const auto abscissa =
        interpolants(TrajectoryIndex::DIST_TO_SF_BWD, Slice());
    const auto t_left = norm_2_interpolant(
        interpolants(Slice(TrajectoryIndex::PX, TrajectoryIndex::PY + 1),
                     Slice()) -
        interpolants(Slice(TrajectoryIndex::LEFT_BOUND_X,
                           TrajectoryIndex::LEFT_BOUND_Y + 1),
                     Slice()))[0];
    const auto t_right =
        -1.0 *
        norm_2_interpolant(
            interpolants(Slice(TrajectoryIndex::PX, TrajectoryIndex::PY + 1),
                         Slice()) -
            interpolants(Slice(TrajectoryIndex::RIGHT_BOUND_X,
                               TrajectoryIndex::RIGHT_BOUND_Y + 1),
                         Slice()))[0];
    const auto left_intp =
        casadi::interpolant("left_intp_impl", "bspline",
                            {abscissa.get_elements()}, t_left.get_elements());
    const auto right_intp =
        casadi::interpolant("right_intp_impl", "bspline",
                            {abscissa.get_elements()}, t_right.get_elements());
    const auto x_intp = casadi::interpolant(
        "x_intp_impl", "bspline", {abscissa.get_elements()},
        interpolants(TrajectoryIndex::PX, Slice()).get_elements());
    const auto y_intp = casadi::interpolant(
        "y_intp_impl", "bspline", {abscissa.get_elements()},
        interpolants(TrajectoryIndex::PY, Slice()).get_elements());
    const auto vel_intp = casadi::interpolant(
        "vel_intp_impl", "bspline", {abscissa.get_elements()},
        interpolants(TrajectoryIndex::SPEED, Slice()).get_elements());

    const auto s = MX::sym("s", 1, 1);
    const auto s_mod =
        utils::align_abscissa<MX>(s, total_length_ / 2.0, total_length_);
    ...
    const auto curvature =
        dx_func(s_mod)[0] * d2y_func(s_mod)[0] -
        dy_func(s_mod)[0] * d2x_func(s_mod)[0] /
            MX::sqrt(MX::pow(MX::pow(dx_func(s_mod)[0], 2) +
                                 MX::pow(dy_func(s_mod)[0], 2),
                             3));
  ...
  // build the frenet to global transformation
  {
    ...
    frenet_to_global_ =
        Function("frenet_to_global", {MX::vertcat({s, t, xi})}, {out});
  }
  ...
  // build the global to frenet transformation
  {
    ...
    const auto s0_mod =
        utils::align_abscissa<MX>(s0, total_length_ / 2.0, total_length_);
    const auto t0 = MX::sym("t0", 1, 1);
    const auto s = MX::sym("s", 1, 1);
    const auto dist_sq = MX::sumsqr(
        frenet_to_global_(MX::vertcat({s, 0.0, 0.0}))[0](Slice(0, 2)) -
        MX::vertcat({x, y}));
    const auto qp =
        casadi::MXDict{{"x", s}, {"f", dist_sq}, {"p", MX::vertcat({x, y})}};
    global_to_frenet_sol_ = casadi::nlpsol(
        ...
        {{"print_time", false},
         ...
         {"print_status", false},
         {"qpsol", "qrqp"},
         {"qpsol_options", casadi::Dict{{"print_header", false},
                                        {"print_iter", false},
                                        {"print_info", false}}}});
    const auto sol =
    ...
    auto s_out = sol.at("x");
    s_out =
        utils::align_abscissa<MX>(s_out, total_length_ / 2.0, total_length_);
    const auto x_out = x_intp_(s_out)[0];
    const auto y_out = y_intp_(s_out)[0];
    const auto yaw_out = yaw_intp_(s_out)[0];
    const auto t_out =
        MX::hypot(x - x_out, y - y_out) *
        utils::lateral_sign<MX>(MX::vertcat({x, y}),
                                MX::vertcat({x_out, y_out, yaw_out}));
    const auto xi_out = utils::align_yaw<MX>(phi, yaw_out) - yaw_out;
    global_to_frenet_ =
        Function("global_to_frenet", {MX::vertcat({x, y, phi, s0, t0})},
...
void RacingTrajectory::frenet_to_global(const FrenetPose2D &frenet_pose,
                                        Pose2D &global_pose) {
  const auto out = frenet_to_global_(casadi::DM{
      frenet_pose.position.s, frenet_pose.position.t, frenet_pose.yaw})[0]
                       .get_elements();
...
void RacingTrajectory::global_to_frenet(const Pose2D &global_pose,
  ...
  if (initialize_with_previous) {
    ...
    const size_t idx = kd_tree_.find_closest_waypoint_index(
        global_pose.position.x, global_pose.position.y);
    ...
    p0.position.t =
        static_cast<double>(distance(global_pose.position, p0_g.position)) *
        lateral_sign(global_pose.position, p0_g);
    p0.yaw = utils::align_yaw(global_pose.yaw, p0_g.yaw);
  }

  const auto out = global_to_frenet_(casadi::DM{
      global_pose.position.x, global_pose.position.y, global_pose.yaw,
      p0.position.s, p0.position.t})[0]
                       .get_elements();
...
const double &RacingTrajectory::total_length() const { return total_length_; }
} // namespace racing_trajectory
} // namespace vehicle_model
} // namespace lmpc
