...
def test_waypoint_bonus_exceeds_max_scaled_speed_reward(tmp_path: Path):
    reward = _reward(
        tmp_path,
        speed_reward_weight=0.1,
        waypoint_bonus_weight=20.0,
    )

    step_reward = reward(_obs(vx=8.0), terminated=False)

    assert step_reward > 8.0


def test_collision_penalty_exceeds_max_step_progress_reward(tmp_path: Path):
    reward = _reward(
        tmp_path,
        speed_reward_weight=0.1,
        waypoint_bonus_weight=20.0,
    ...
    assert reward(_obs(vx=8.0, collision=True), terminated=True) == -50.0
...
def test_default_ppo_config_uses_convergence_safe_settings():
    ...
    assert config.reward["waypoint_bonus_weight"] == 20.0
    assert config.reward["collision_penalty"] == 50.0
    assert config.training.c2 == 0.0
