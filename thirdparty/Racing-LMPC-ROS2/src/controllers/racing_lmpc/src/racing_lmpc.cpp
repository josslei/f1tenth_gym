...
RacingLMPC::RacingLMPC(RacingLMPCConfig::SharedPtr mpc_config,
    ...
    : config_(mpc_config), model_(model),
      ...
      opti_(casadi::Opti()), X_(opti_.variable(model_->nx(), config_->N)),
      ...
      vel_ref_(opti_.parameter(1, config_->N)), solved_(false), sol_() {
  using casadi::MX;
  using casadi::Slice;

  // configure solver
  const auto p_opts = casadi::Dict{
      {"expand", true}, {"print_time", config_->verbose ? true : false}};
  const auto s_opts =
      casadi::Dict{{"max_cpu_time", config_->max_cpu_time},
                   {"tol", config_->tol},
  ...
  const auto margin =
      config_->margin + model_->get_base_config().chassis_config->b / 2.0;
  if (enable_boundary_slack) {
    boundary_slack_ = opti_.variable(1, config_->N);
    opti_.subject_to(opti_.bounded(bound_right_ + margin - boundary_slack_, PY,
                                   bound_left_ - margin + boundary_slack_));
    opti_.subject_to(boundary_slack_ >= 0.0);
    cost +=
        MX::mtimes({boundary_slack_, config_->q_boundary, boundary_slack_.T()});
  } else {
    opti_.subject_to(
        opti_.bounded(bound_right_ + margin, PY, bound_left_ - margin));
  }
  ...
  for (size_t i = 0; i < config_->N - 1; i++) {
    ...
    // xi start with 1 since x0 must equal to x_ic and there is nothing we can
    // do about it
    if (i >= 1) {
      // const auto d_px =
      // utils::align_abscissa<MX>(xi(XIndex::PX), x0(XIndex::PX),
      // total_length_) - x0(XIndex::PX);
      const auto x_base =
          model_->to_base_state()(casadi::MXDict{{"x", xi}, {"u", ui}})
              .at("x_out");
      const auto dv =
          hypot(x_base(XIndex::VX), x_base(XIndex::VY)) - vel_ref_(i);
      /*
      const auto px_dot = (xi(XIndex::VX) * cos(xi(XIndex::YAW)) -
      xi(XIndex::VY) * sin(xi(XIndex::YAW))) / (1 - xi(XIndex::PY) *
      curvatures_(i)); const auto py_dot = xi(XIndex::VX) * sin(xi(XIndex::YAW))
      + xi(XIndex::VY) * cos(xi(XIndex::YAW)); const auto heading_diveation =
      atan2(py_dot, px_dot); const auto beta_phi = atan2(py_dot, px_dot);
      */
  ...
  const auto dv = hypot(x_base_N(XIndex::VX), x_base_N(XIndex::VY)) -
                  vel_ref_(config_->N - 1);
  /*
  const auto px_dot = xN(XIndex::VX) * cos(xN(XIndex::YAW)) - xN(XIndex::VY) *
  sin(xN(XIndex::YAW)) / (1 - xN(XIndex::PY) * curvatures_(config_->N - 1));
  const auto py_dot = xN(XIndex::VX) * sin(xN(XIndex::YAW)) + xN(XIndex::VY) *
  cos(xN(XIndex::YAW)); const auto heading_diveation = atan2(py_dot, px_dot);
  */
  ...
  for (size_t i = 0; i < config_->N - 1; i++) {
    ...
    casadi::MXDict constraint_in = {
        {"x", xi}, {"u", ui}, {"xip1", xip1},
        {"t", ti}, {"k", k},  {"track_length", total_length_}};
    if (i < config_->N - 2) {
    ...
    if (i == 0) {
      casadi::MXDict constraint_in_0 = {{"u", u_ic_}, {"uip1", ui}, {"t", ti}};
...
void RacingLMPC::solve(const casadi::DMDict &in, casadi::DMDict &out,
                       casadi::Dict &stats) {
  using casadi::DM;
  using casadi::MX;
  using casadi::Slice;
  ...
  X_ref(XIndex::PX, Slice()) =
      align_abscissa_(
          casadi::DMDict{
              {"abscissa_1", X_ref(XIndex::PX, Slice())},
              {"abscissa_2", DM::ones(1, config_->N) * x_ic(XIndex::PX)},
              {"total_distance", DM::ones(1, config_->N) * total_length}})
          .at("abscissa_1_aligned");
  ...
  if (in.count("X_optm_ref")) {
    ...
    X_optm_ref(XIndex::PX, Slice()) =
        align_abscissa_(
            casadi::DMDict{
                {"abscissa_1", X_optm_ref(XIndex::PX, Slice())},
                {"abscissa_2", DM::ones(1, config_->N) * x_ic(XIndex::PX)},
                {"total_distance", DM::ones(1, config_->N) * total_length}})
            .at("abscissa_1_aligned");
    ...
    if (!sol_) {
      throw std::runtime_error(
          "No warm start given and no previous solution found.");
    }
    opti_.set_value(T_ref_, in.at("T_ref"));
    opti_.set_initial(sol_->value_variables());
    const auto last_abscissa_optm =
        (sol_->value(X_) * scale_x_)(XIndex::PX, Slice());
    ...
    const auto this_abscissa_optm =
        align_abscissa_(casadi::DMDict{{"abscissa_1", last_abscissa_optm},
                                       {"abscissa_2", P0},
                                       {"total_distance", total_lengths}})
            .at("abscissa_1_aligned");
...
void RacingLMPC::create_warm_start(const casadi::DMDict &in,
                                   casadi::DMDict &out) {
  using casadi::DM;
  using casadi::Slice;
  ...
  if (static_cast<size_t>(P0.size2()) != config_->N) {
    throw std::length_error(
        "create_warm_start: P0 dimension does not match MPC dimension.");
  }
  if (static_cast<size_t>(Yaws.size2()) != config_->N) {
    throw std::length_error(
        "create_warm_start: Yaws dimension does not match MPC dimension.");
  }
  if (static_cast<double>(current_vel) <= 0.0) {
    throw std::range_error(
        "Current velocity cannot be smaller than or equal to zero.");
  }
  if (static_cast<double>(target_vel) <= 0.0) {
    throw std::range_error(
        "Target velocity cannot be smaller than or equal to zero.");
  }
  ...
  X_ref(XIndex::VX, Slice()) =
      DM::linspace(current_vel, target_vel, config_->N);
  ...
  for (size_t i = 0; i < config_->N - 1; i++) {
    ...
    {
      U_ref(UIndex::STEER, i) =
          atan(model_->get_base_config().chassis_config->wheel_base / Radii(i));
    }
  }
  out["X_ref"] = X_ref;
  out["U_ref"] = U_ref;
...
BaseVehicleModel &RacingLMPC::get_model() { return *model_; }

const bool &RacingLMPC::solved() const { return solved_; }
} // namespace racing_lmpc
} // namespace mpc
} // namespace lmpc
