...
def plot_tpamap_fromFile(
    track_name: str, filename_tpamap: str, filename_frictiondata: str
) -> None:
    """
    Documentation
    ...
    filepath_frictionmap = os.path.join(
        path2module, "inputs", "frictionmaps", filename_tpamap
    )
    filepath_frictiondata = os.path.join(
        path2module, "inputs", "frictionmaps", filename_frictiondata
    )
    ...
    trackbound_right, trackbound_left = (
        frictionmap.src.reftrack_functions.calc_trackboundaries(reftrack=reftrack)
    )
    ...
    plot_tpamap_fromVariable(
        tpa_map=tpamap_loaded,
...
def plot_tpamap_fromVariable(
    tpa_map: cKDTree,
    tpa_data: dict,
    refline: np.ndarray,
    trackbound_right: np.ndarray,
    trackbound_left: np.ndarray,
) -> None:
    """
    Documentation
    ...
    if min(list_mue) == max(list_mue):
        ...
        con = plt.contourf(
            ...
            z,
            np.arange(
                np.round(min(list_mue) - 0.05, 1),
                np.round(max(list_mue) + 0.06, 1) + 0.01,
                0.02,
            ),
        )
        cbar_tickrange = np.arange(
            np.round(min(list_mue) - 0.05, 1),
            np.round(max(list_mue) + 0.06, 1) + 0.01,
            0.05,
        )
