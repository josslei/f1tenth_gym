...
//
// F_{r,x}/m is exactly our control's "a" (acceleration is the control's own
// physical definition), so v_dot_x = a directly.
//
...
// which is what makes the c_du||u_t - u_{t-1}||^2 cost term possible in the
// first place) -- this needs dt, hence the constructor argument that
