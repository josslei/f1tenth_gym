# pyright: reportPossiblyUnboundVariable=false, reportAttributeAccessIssue=false, reportOptionalMemberAccess=false
# (suppressed: gauss_model/w_mue_* unbound in conditional branches;
#  coef_/intercept_ on Pipeline._final_estimator are runtime-known attributes)
...
def approx_friction_map(
    ...
    import opt_mintime_traj.src.extract_friction_coeffs as extract_friction_coeffs

    n, mue_fl, mue_fr, mue_rl, mue_rr = extract_friction_coeffs.extract_friction_coeffs(
        reftrack=reftrack,
    ...
    for i in range(num_steps):
        if pars["optim_opts"]["var_friction"] == "linear":
            ...
            gauss_model = make_pipeline(
                GaussianFeatures(n_gauss_tot), LinearRegression()
            )
