...
namespace lmpc {
namespace utils {
template <typename T> T align_yaw(const T &yaw_1, const T &yaw_2) {
...
T global_to_frenet(const T &p, const T &p0, const T &yaw) {
  ...
  const auto R = T(T::vertcat({T::horzcat({cos_theta, -sin_theta}),
...
template <typename T> T lateral_sign(const T &position, const T &p0) {
  using casadi::Slice;
  return sign(cos(p0(2, Slice())) * (position(1, Slice()) - p0(1, Slice())) -
 ...
 * @param nu size of control
 * @param dynamics continuous dynamics function
 * @return casadi::Function with inputs `x`, `u` and `dt` and outputs next state
 * `xip1`.
 */
casadi::Function rk4_function(const casadi_int &nx, const casadi_int &nu,
                              const casadi::Function &dynamics);
...
enum TyreIndex : size_t { FL = 0, FR = 1, RL = 2, RR = 3 };
} // namespace utils
} // namespace lmpc
