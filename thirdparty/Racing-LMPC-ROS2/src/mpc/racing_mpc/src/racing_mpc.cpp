...
RacingMPC::RacingMPC(RacingMPCConfig::SharedPtr mpc_config,
    ...
    : config_(mpc_config), model_(model),
      ...
      ss_recorder_(std::make_unique<SafeSetRecorder>(
          *ss_manager_, config_->record, config_->path_prefix)) {
  using casadi::MX;
  using casadi::Slice;

  // configure solver
  if (full_dynamics) {
    auto p_opts = casadi::Dict{{"expand", true},
  ...
  for (size_t i = 0; i < config_->N - 1; i++) {
    ...
    if (full_dynamics) {
      ...
      const auto AB = model_->discrete_dynamics_jacobian()(
...
const RacingMPCConfig &RacingMPC::get_config() const { return *config_.get(); }
...
void RacingMPC::solve(const casadi::DMDict &in, casadi::DMDict &out,
  ...
  if (ss_result.x.size2() == 0) {
    ...
    if (ss_x.size2() < config_->num_ss_pts) {
      ...
      ss_x = casadi::DM::horzcat(
          {ss_x, casadi::DM::repmat(ss_x(Slice(), -1), 1,
                                    config_->num_ss_pts - ss_x.size2())});
      ss_j = casadi::DM::horzcat(
          {ss_j, casadi::DM::repmat(ss_j(Slice(), -1), 1,
                                    config_->num_ss_pts - ss_j.size2())});
  ...
  try {
    ...
    if (config_->learning) {
      ...
      // std::cout << DM::mtimes(out["ss_x"],
      // out["convex_combi_optm"])(XIndex::VX) << std::endl;
    }
...
void RacingMPC::create_warm_start(const casadi::DMDict &in,
                                  casadi::DMDict &out) {
  using casadi::DM;
  using casadi::Slice;
...
BaseVehicleModel &RacingMPC::get_model() { return *model_; }

const bool &RacingMPC::solved() const { return solved_; }
...
void RacingMPC::build_tracking_cost(casadi::MX &cost) {
  ...
  for (size_t i = 0; i < config_->N - 1; i++) {
    ...
    // do about it const auto d_px = utils::align_abscissa<MX>(xi(XIndex::PX),
    // x0(XIndex::PX), total_length_) - x0(XIndex::PX);
    const auto x_base =
  ...
  cost +=
      x_base_N(XIndex::PY) * x_base_N(XIndex::PY) * config_->q_contour * 10.0;
  cost +=
      x_base_N(XIndex::YAW) * x_base_N(XIndex::YAW) * config_->q_heading * 10.0;
...
void RacingMPC::build_lmpc_cost(casadi::MX &cost) {
  ...
  if (enable_convex_hull_slack) {
    ...
    cost +=
        MX::mtimes({convex_hull_slack_.T(),
                    MX::diag(config_->convex_hull_slack), convex_hull_slack_});
  } else {
  ...
  for (size_t i = 0; i < config_->N - 1; i++) {
    ...
    // const auto xi = X_(Slice(), i) * scale_x_;
    // const auto x_base =
    //   model_->to_base_state()(casadi::MXDict{{"x", xi}, {"u",
    //   ui}}).at("x_out");
    // cost += x_base(XIndex::VY) * x_base(XIndex::VY) * config_->q_vy;
    // cost += x_base(XIndex::VYAW) * x_base(XIndex::VYAW) * config_->q_vyaw;
...
void RacingMPC::build_boundary_constraint(casadi::MX &cost) {
  ...
  if (enable_boundary_slack) {
    ...
    cost +=
        MX::mtimes({boundary_slack_.T(), config_->q_boundary, boundary_slack_});
  } else {
    opti_.subject_to(
