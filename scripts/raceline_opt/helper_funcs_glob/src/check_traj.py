...
def check_traj(
    ...
    bound_r_interp = helper_funcs_glob.src.interp_track.interp_track(
        reftrack=bound_r_tmp, stepsize_approx=1.0
    )[0]
    bound_l_interp = helper_funcs_glob.src.interp_track.interp_track(
        reftrack=bound_l_tmp, stepsize_approx=1.0
    )[0]

    # calculate minimum distances of every trajectory point to the boundaries
    min_dists = helper_funcs_glob.src.calc_min_bound_dists.calc_min_bound_dists(
