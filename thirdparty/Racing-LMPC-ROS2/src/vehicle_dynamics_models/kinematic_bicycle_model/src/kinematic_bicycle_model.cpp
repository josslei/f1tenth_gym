...
size_t KinematicBicycleModel::nx() const { return 4; }

size_t KinematicBicycleModel::nu() const { return 3; }

void KinematicBicycleModel::add_nlp_constraints(casadi::Opti &opti,
  ...
  if (in.count("x")) {
    ...
    const auto xip1_pred =
        discrete_dynamics_({{"x", x}, {"u", u}, {"k", k}, {"dt", t}})
            .at("xip1");
  ...
  if (in.count("uip1")) {
    ...
    opti.subject_to(opti.bounded(-delta_max / Tdelta,
                                 (uip1(UIndex::STEER) - delta) / t,
                                 delta_max / Tdelta));
  }
}

void KinematicBicycleModel::calc_lon_control(const casadi::DMDict &in,
                                             double &throttle,
...
void KinematicBicycleModel::calc_lat_control(const casadi::DMDict &in,
...
void KinematicBicycleModel::compile_dynamics() {
  ...
  // - front const auto & tyre_r = *get_base_config().rear_tyre_config; const
  // auto & Br = tyre_r.pacejka_b;  // magic formula B - rear const auto & Cr =
  // tyre_r.pacejka_c;  // magic formula C - rear const auto & Er =
  // tyre_r.pacejka_e;  // magic formula E - rear const auto & Fz0_r =
  // tyre_r.pacejka_fz0;  // magic formula Fz0 - rear const auto & eps_r =
  // tyre_r.pacejka_eps;  // extended magic formula epsilon - rear

  // compute kinematics
  const auto beta = atan(lr * tan(delta) / l);
  auto phi_dot = v * tan(delta) * cos(beta) / l;
  ...
  dynamics_ = casadi::Function("kinematic_bicycle_model", {x, u, k},
                               {x_dot, Fx_ij, Fz_ij}, {"x", "u", "k"},
                               {"x_dot", "Fx_ij", "Fz_ij"});
  ...
  dynamics_jacobian_ =
      casadi::Function("kinematic_bicycle_model_jacobian", {x, u, k}, {Ac, Bc},
                       {"x", "u", "k"}, {"A", "B"});
  ...
  discrete_dynamics_ = casadi::Function(
      ...
      {xip1, Fx_ij, Fz_ij}, {"x", "u", "k", "dt"}, {"xip1", "Fx_ij", "Fz_ij"});
  ...
  const auto base_x = SX{vertcat(
      ...
      x(kinematic_bicycle_model::XIndex::V) * sin(beta), global_yaw_rate)};
  to_base_state_ = casadi::Function("to_base_state", {x, u}, {base_x},
                                    {"x", "u"}, {"x_out"});

  const auto base_x_sym = SX::sym("base_x", BaseVehicleModel::nx());
  const auto base_u_sym = SX::sym("base_u", BaseVehicleModel::nu());
  const auto derived_x =
      SX{vertcat(base_x_sym(base_vehicle_model::XIndex::PX),
                 base_x_sym(base_vehicle_model::XIndex::PY),
                 base_x_sym(base_vehicle_model::XIndex::YAW),
                 hypot(base_x_sym(base_vehicle_model::XIndex::VX),
                       base_x_sym(base_vehicle_model::XIndex::VY)))};
  from_base_state_ =
      casadi::Function("from_base_state", {base_x_sym, base_u_sym}, {derived_x},
                       {"x", "u"}, {"x_out"});
}
} // namespace kinematic_bicycle_model
} // namespace vehicle_model
