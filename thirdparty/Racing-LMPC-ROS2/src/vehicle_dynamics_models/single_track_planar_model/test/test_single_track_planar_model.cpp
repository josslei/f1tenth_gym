...
TEST(SingleTrackPlanarModelTest, TestSingleTrackPlanarModel) {
  ...
  options.arguments({
      "--ros-args",
      "--params-file",
      base_share_dir + "/param/sample_vehicle.param.yaml",
      "--params-file",
      share_dir + "/param/sample_vehicle.param.yaml",
  });
  ...
  auto config = lmpc::vehicle_model::single_track_planar_model::load_parameters(
      &test_node);
  auto model =
      lmpc::vehicle_model::single_track_planar_model::SingleTrackPlanarModel(
          base_config, config);
...
TEST(SingleTrackPlanarModelTest, TestSingleTrackDynamics) {
  ...
  const auto in = casadi::DMDict{{"x", x}, {"u", u}, {"k", 0.1}};
