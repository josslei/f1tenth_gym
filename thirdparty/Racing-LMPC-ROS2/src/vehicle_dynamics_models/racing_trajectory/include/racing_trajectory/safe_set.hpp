...
struct RegQuery {
  ...
  casadi::DM x;                // stacked local query point [x; u]
  ...
  casadi_int max_num_total;    // maximum number of points to return
  casadi_int max_num_per_lap;  // maximum number of points to return per lap
  Indices reg_in_state_idxs;   // indices of the state variables to be used for
                               // regression
  Indices reg_in_control_idxs; // indices of the control variables to be used
                               // for regression
  Indices
      reg_out_state_idxs; // indices of the output variables of the regression
};
...
struct RegResult {
  ...
  casadi::DM xs_full; // full-state query results for nominal prediction
  casadi::DM us_full; // full-control query results for nominal prediction
...
public:
  ...
  explicit SSTrajectory(const casadi::DM &x, const casadi::DM &u,
...
private:
  ...
  SSTrajectoryData process_lap_data(const casadi::DM &x, const casadi::DM &u,
                                    const casadi::DM &k, const casadi::DM &t,
...
public:
  ...
  void add_lap(const casadi::DM &x, const casadi::DM &u, const casadi::DM &k,
...
public:
  ...
  explicit SafeSetRecorder(SafeSetManager &manager, const bool &to_file,
                           const std::string &file_prefix);
  void step(const casadi::DM &x, const casadi::DM &u, const casadi::DM &k,
            const casadi::DM &t, const double &total_length);

  void load(const std::vector<std::string> &from_files,
            const double &total_length);
