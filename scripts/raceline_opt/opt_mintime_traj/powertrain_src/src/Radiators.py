...
class RadiatorModel:
    ...
    def initialize(self):
        ...
        self.temp_cool_mi_n = ca.SX.sym("temp_cool_mi_n")  # pyright: ignore[reportArgumentType]
        ...
        self.temp_cool_b_n = ca.SX.sym("temp_cool_b_n")  # pyright: ignore[reportArgumentType]
    ...
    def get_intermediate_temps(self, temp_inv: ca.SX, r_inv: float):
        """
        Python version: 3.5
        Created by: Thomas Herrmann
        Created on: 01.04.2020
        ...
        self.temp_cool_12 = (
            self.temp_cool_mi
            * (self.pars["c_heat_fluid"] * self.pars["flow_rate_inv"] * r_inv - 1)
            + 2 * temp_inv
        ...
        self.temp_cool_13 = (
            self.temp_cool_mi
            * (
                2 * self.pars["c_heat_fluid"] * self.pars["flow_rate_rad"] * self.r_rad
                + 1
            )
            - 2 * self.pars["T_env"]
        ) / (
    ...
    def get_increment_mi(
        ...
        self.dtemp_cool_mi = sf * (
            (
                self.pars["N_machines"]
                * (
                    (temp_mot - (self.temp_cool_12 + self.temp_cool_13) / 2) / r_machine
                    + (temp_inv - (self.temp_cool_mi + self.temp_cool_12) / 2) / r_inv
                )
                - ((self.temp_cool_mi + self.temp_cool_13) / 2 - self.pars["T_env"])
                / self.r_rad
            )
    ...
    def get_increment_b(
        self, sf: ca.SX, temp_batt: ca.SX, temp_cool_b: ca.SX, R_eq_B_inv: float
    ):
        """
        Python version: 3.5
        Created by: Thomas Herrmann
        ...
        self.dtemp_cool_b = sf * (
            (
                R_eq_B_inv * (temp_batt - temp_cool_b)
    ...
    def ini_nlp_state(self, x: ca.SX, u: ca.SX):
        ...
        self.f_nlp = ca.Function(
            "f_nlp",
            [x, u],
            [self.temp_cool_mi, self.temp_cool_b],
            ["x", "u"],
            ["temp_cool_mi", "temp_cool_b"],
        )
    ...
    def extract_sol(self, w: ca.SX, sol_states: ca.DM):
        ...
        self.temp_cool_mi = temps_opt[0::2]  # pyright: ignore[reportOptionalSubscript]
        self.temp_cool_b = temps_opt[1::2]  # pyright: ignore[reportOptionalSubscript]
