...
def friction_map_plot(
    filepath_tpamap: str, filepath_tpadata: str, filepath_referenceline: str
):
    """
    Created by:
    ...
    print(
        "INFO: Preprocessing friction map data for visualization... (takes some time)"
    )
    ...
    x_vals = np.arange(
        x_min - (20.0 * x_stepsize), x_max + (19.0 * x_stepsize), x_stepsize
    )
    y_vals = np.arange(
        y_min - (20.0 * y_stepsize), y_max + (19.0 * y_stepsize), y_stepsize
    )
...
if __name__ == "__main__":
    ...
    tpamap_path = os.path.join(
        module_path, "inputs", "frictionmaps", "berlin_2018_tpamap.csv"
    )
    tpadata_path = os.path.join(
        module_path, "inputs", "frictionmaps", "berlin_2018_varmue08-12_tpadata.json"
    )
    referenceline_path = os.path.join(
        module_path, "inputs", "tracks", "berlin_2018.csv"
    )

    friction_map_plot(
        filepath_tpamap=tpamap_path,
