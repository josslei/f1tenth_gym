# pyright: reportPossiblyUnboundVariable=false, reportAssignmentType=false, reportAttributeAccessIssue=false, reportOptionalSubscript=false
# (suppressed: temp_* vars defined conditionally; fig.gca() type narrowed by matplotlib stubs;
#  pwr subscript on None-typed param — always a dict when pwr_behavior is True)
...
def result_plots_mintime(
    ...
    if plot_opts["general"]:
        ...
        plt.xlabel("distance " + r"$\it{s}$" + " in " + r"$\it{m}$")
        plt.ylabel(
            "relative angle to tangent on reference line "
            + r"$\xi$"
            + " in "
            + r"$\it{°}$"
        )
    ...
    if plot_opts["lateral_distance"]:
        ...
        plt.plot(
            s, reftrack_cl[:, 3] - pars["optim_opts"]["width_opt"] / 2, color="grey"
        )
        plt.plot(s, -reftrack_cl[:, 2], color="black")
        plt.plot(
            s, -reftrack_cl[:, 2] + pars["optim_opts"]["width_opt"] / 2, color="grey"
        )
        plt.xlabel("distance " + r"$\it{s}$" + " in " + r"$\it{m}$")
        plt.ylabel(
            "lateral distance to reference line " + r"$\it{n}$" + " in " + r"$\it{m}$"
        )
        plt.legend(
            ["raceline", "road boundaries", "road boundaries - safety margin"],
            ncol=1,
            loc=4,
        )
        plt.grid()
    ...
    if plot_opts["kamm_circle"]:
        ...
        plt.plot(
            tf_y_fl / (tf_z_fl * pars["optim_opts"]["mue"]),
        ...
        ax.add_artist(circle1)
        plt.plot(
            tf_y_fr / (tf_z_fr * pars["optim_opts"]["mue"]),
        ...
        ax.add_artist(circle1)
        plt.plot(
            tf_y_rl / (tf_z_rl * pars["optim_opts"]["mue"]),
        ...
        ax.add_artist(circle1)
        plt.plot(
            tf_y_rr / (tf_z_rr * pars["optim_opts"]["mue"]),
    ...
    if plot_opts["tire_forces_dynamic"]:
        ...
        f_zlong_fl = (
            ...
            * veh["cog_z"]
            / veh["wheelbase"]
            * (f_drive + f_brake - f_xroll - f_xdrag)
        )
        f_zlong_fr = (
            -0.5
            * veh["cog_z"]
        ...
        f_zlong_rl = (
            ...
            * veh["cog_z"]
            / veh["wheelbase"]
            * (f_drive + f_drive - f_xroll - f_xdrag)
        )
        f_zlong_rr = (
            0.5
            * veh["cog_z"]
    ...
    if pars["pwr_params_mintime"]["pwr_behavior"] and plot_opts["pwr_states"]:
        ...
        plt.legend(
            [
                r"$\it{T_\mathrm{Machine}}$",
                r"$\it{T_\mathrm{Battery}}$",
                r"$\it{T_\mathrm{Inverter}}$",
                r"$\it{T_\mathrm{Fluid_{MI}}}$",
                r"$\it{T_\mathrm{Fluid_B}}$",
            ]
        )
        plt.grid()
        plt.show()
    ...
    if pars["pwr_params_mintime"]["pwr_behavior"] and plot_opts["pwr_losses"]:
        if pars["pwr_params_mintime"]["simple_loss"]:
            ...
            plt.legend(
                [
                    r"$\it{P_\mathrm{loss,machine}}$",
                    r"$\it{P_\mathrm{loss,inverter}}$",
            ...
            plt.legend(
                [
                    r"$\it{P_\mathrm{loss,total}}$",
                    r"$\it{P_\mathrm{loss,copper}}$",
                    r"$\it{P_\mathrm{loss,statorIron}}$",
                    r"$\it{P_\mathrm{loss,rotor}}$",
                ]
            )
            plt.grid()
            plt.subplot(312)
            ...
            plt.legend(
                [
                    r"$\it{P_\mathrm{loss,total}}$",
                    r"$\it{P_\mathrm{loss,switching}}$",
                    r"$\it{P_\mathrm{loss,conducting}}$",
                ]
            )
            plt.ylabel(
                "Power loss single inverter\n"
                + r"$\it{P_\mathrm{loss}}$"
                + " in "
                + r"kW"
            )
