...
enum UIndex : casadi_int { FD = 0, FB = 1, STEER = 2 };
...
public:
   ...
   *  In the input, "x" (state) and "u" (control) are usually required.
   * additional inputs are optional. In the output, "x_dot" (time derivative of
   * state) is usually required. additional outputs are optional.
   */
  virtual const casadi::Function &dynamics() const;

  /**
   * @brief Returns the jacobian of the continuious dynamics.
   * In the function, the inputs typically include "x" (state) and "u"
   * (control). The function outputs Jacobian of "x_dot" with respect to x (A
   * matrix) and u (B matrix) evaulated at their given point.
   */
   ...
   *  In the input, "x" (state) and "u" (control) are usually required.
   * additional inputs are optional. In the output, "xip1" (next state) is
   * usually required. additional outputs are optional. The discretization
   * method specified should be as in the config.
   ...
   * In the function, the inputs typically include "x" (state) and "u"
   * (control). The function outputs Jacobian of "xip1" with respect to x (A
   * matrix) and u (B matrix) evaulated at their given point.
   */
   ...
   *
   * @return const casadi::Function& function that converts "x" and "u" to the
   * base state variable.
   */
   ...
   *        this function should take "x" and "u",
   *        and return a function that converts them to the base control
   * variable.
   *
   * @return const casadi::Function& function that converts "x" and "u" to the
   ...
   * @param in "x" (state), "u" (control), "t"(delta t), "xip1" (next state),
   * "uip1" (next control).
   */
  virtual void add_nlp_constraints(casadi::Opti &opti,
   ...
   */
  virtual void calc_lon_control(const casadi::DMDict &in, double &throttle,
   ...
   */
  virtual void calc_lat_control(const casadi::DMDict &in,
