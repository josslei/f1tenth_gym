...
TrajectoryKDTree::TrajectoryKDTree(const std::vector<double> &x,
...
size_t TrajectoryKDTree::find_closest_waypoint_index(const double &x,
                                                     const double &y) const {
  Point_d query(x, y);
  Neighbor_search search(tree_, query, 1);
  const auto &closest_pt = *search.begin();
  return map_.find(WaypointKey{closest_pt.first.x(), closest_pt.first.y()})
      ->second;
}
...
void TrajectoryKDTree::find_closest_waypoint_indices(
  ...
  for (const auto &pt : search) {
    indices.push_back(
        map_.find(WaypointKey{pt.first.x(), pt.first.y()})->second);
  }
}

void TrajectoryKDTree::get_waypoint(const size_t &index, double &x,
                                    double &y) const {
  x = x_[index];
  y = y_[index];
}
