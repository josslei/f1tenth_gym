...
RacingLMPCNode::RacingLMPCNode(const rclcpp::NodeOptions &options)
    : rclcpp::Node("racing_lmpc_node", options),
      ...
      model_(vehicle_model::vehicle_model_factory::load_vehicle_model(
          utils::declare_parameter<std::string>(
              this, "racing_lmpc_node.vehicle_model_name"),
          this)),
  ...
  vehicle_state_sub_ =
      this->create_subscription<mpclab_msgs::msg::VehicleStateMsg>(
          "vehicle_state", 1,
          std::bind(&RacingLMPCNode::on_new_state, this, std::placeholders::_1),
          sub_options);
  ...
  if (config_->step_mode == RacingLMPCStepMode::CONTINUOUS) {
    ...
    step_timer_ = this->create_wall_timer(
        ...
        std::bind(&RacingLMPCNode::on_step_timer, this));
  }
}

void RacingLMPCNode::on_new_state(
...
void RacingLMPCNode::on_step_timer() {
  ...
  const auto x_ic_base =
      DM{p.s, p.x_tran, p.e_psi, v.v_long, v.v_tran, w.w_psi};
  state_msg_mutex_.unlock();
  ...
  // const auto mpc_duration =
  // std::chrono::duration_cast<std::chrono::milliseconds>(
  //   mpc_stop - mpc_start);
  // std::cout << "MPC Execution Time: " << mpc_duration.count() << "ms" <<
  // std::endl; if (mpc_->solved()) {
