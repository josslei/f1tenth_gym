...
# (suppressed: pwr is None-typed but always a dict when pwr_behavior is True)

import numpy as np
import os


def export_mintime_solution(
    file_path: str,
    pars: dict,
    s: np.ndarray,
    t: np.ndarray,
    ...
    if pars["pwr_params_mintime"]["pwr_behavior"]:
        ...
        np.savetxt(
            os.path.join(file_path, "states.csv"), states, fmt=fmt_x, header=header_x
        )
    else:
    ...
    np.savetxt(
        os.path.join(file_path, "controls.csv"), controls, fmt=fmt_u, header=header_u
    )

    # save tire forces
    header_tf = (
        "s_m; t_s; f_x_fl_N; f_y_fl_N; f_z_fl_N; f_x_fr_N; f_y_fr_N; f_z_fr_N;"
    ...
    np.savetxt(
        os.path.join(file_path, "tire_forces.csv"),
        tire_forces,
        fmt=fmt_tf,
        header=header_tf,
    )
    ...
    np.savetxt(
        os.path.join(file_path, "accelerations.csv"),
        accelerations,
        fmt=fmt_a,
        header=header_a,
    )
    ...
    if pars["pwr_params_mintime"]["pwr_behavior"]:
        if pars["pwr_params_mintime"]["simple_loss"]:
            ...
            fmt_pwr_l = "%.1f; %.3f; %.2f; %.2f; %.2f; %.2f"
            pwr_losses = np.column_stack(
                (
                    s[:-1],
                    t[:-1],
                    pwr["machine"].p_loss_total,
                    pwr["inverter"].p_loss_total,
                    pwr["batt"].p_loss_total,
                    pwr["batt"].p_out_batt,
                )
            )
        else:
            ...
            fmt_pwr_l = (
                "%.1f; %.3f; %.2f; %.2f; %.2f; %.2f; %.2f; %.2f; %.2f; %.2f; %.2f"
            )
            pwr_losses = np.column_stack(
                (
                    s[:-1],
                    t[:-1],
                    pwr["machine"].p_loss_total,
                    pwr["machine"].p_loss_copper,
                    pwr["machine"].p_loss_stator_iron,
                    pwr["machine"].p_loss_rotor,
                    pwr["inverter"].p_loss_total,
                    pwr["inverter"].p_loss_switch,
                    pwr["inverter"].p_loss_cond,
                    pwr["batt"].p_loss_total,
                    pwr["batt"].p_out_batt,
                )
            )
        ...
        np.savetxt(
            os.path.join(file_path, "power_losses.csv"),
            pwr_losses,
            fmt=fmt_pwr_l,
            header=header_pwr_l,
        )
