...
VanillaController::VanillaController(
    ...
    BaseVehicleModel::SharedPtr model, RacingTrajectory::SharedPtr track)
    : config_(controller_config), model_(model), track_(track),
      pid_controller_("lon_pid", config_->lon_pid_coeffs) {}

const VanillaControllerConfig &VanillaController::get_config() const {
  return *config_.get();
}

void VanillaController::solve(const casadi::DMDict &in, casadi::DMDict &out,
                              casadi::Dict &stats) {
  (void)stats;
  ...
  const auto lookahead_dist = std::clamp(v * config_->lookahead_speed_ratio,
                                         config_->min_lookahead_distance,
                                         config_->max_lookahead_distance);
  const auto s_lookahead = utils::align_abscissa<casadi::DM>(
      s + lookahead_dist, track_->total_length() / 2.0, track_->total_length());
  FrenetPose2D lookahead_frenet_pose{{static_cast<double>(s_lookahead), 0.0},
                                     0.0};
  Pose2D lookahead_global_pose;
  track_->frenet_to_global(lookahead_frenet_pose, lookahead_global_pose);

  // compute pure pursuit
  const auto lookahead_direction =
      atan2(lookahead_global_pose.position.y - current_global_pose.position.y,
  ...
  const auto alpha = utils::TransformHelper::calc_yaw_difference(
      current_global_pose.yaw, lookahead_direction);
  const auto delta =
      atan(2.0 * chassis->wheel_base * sin(alpha) / lookahead_dist);
  const auto delta_clamped =
      std::clamp(delta, -steer->max_steer, steer->max_steer);
  ...
  const auto aero_resistance =
      0.5 * aero->air_density * aero->frontal_area * aero->drag_coeff * v * v;
  const auto down_force = aero_resistance * (aero->cl_f + aero->cl_r);
  const auto rolling_resistance =
      chassis->fr * (chassis->total_mass * GRAVITY + down_force);
  const auto ctrl_force =
      chassis->total_mass * acc + rolling_resistance + aero_resistance;
  if (ctrl_force > 0.0) {
...
BaseVehicleModel &VanillaController::get_model() { return *model_; }
} // namespace vanilla_controller
} // namespace mpc
} // namespace lmpc
