...
const auto share_dir =
    ament_index_cpp::get_package_share_directory("racing_lmpc");
const auto base_share_dir =
    ament_index_cpp::get_package_share_directory("base_vehicle_model");
const auto model_share_dir =
    ament_index_cpp::get_package_share_directory("single_track_planar_model");
const auto trajectory_share_dir =
    ament_index_cpp::get_package_share_directory("racing_trajectory");
RacingLMPC::SharedPtr get_mpc() {
  rclcpp::init(0, nullptr);
  rclcpp::NodeOptions options;
  options.arguments(
      {"--ros-args", "--params-file",
       base_share_dir + "/param/sample_vehicle_2.param.yaml", "--params-file",
       model_share_dir + "/param/sample_vehicle_2.param.yaml", "--params-file",
       share_dir + "/param/sample_mpc_2.param.yaml"});
  auto test_node = rclcpp::Node("test_racing_lmpc_node", options);

  auto base_config =
      lmpc::vehicle_model::base_vehicle_model::load_parameters(&test_node);
  auto model_config =
      lmpc::vehicle_model::single_track_planar_model::load_parameters(
          &test_node);
  auto model =
      std::make_shared<SingleTrackPlanarModel>(base_config, model_config);
...
void test_mpc(const lmpc::Pose2D &p0, const double &v0,
              const casadi_int &num_step) {
  using casadi::DM;
  using casadi::Slice;
  auto mpc = get_mpc();
  ...
  auto traj =
      lmpc::vehicle_model::racing_trajectory::RacingTrajectory(test_traj_file);
  ...
  const auto x_ic = DM{
      x0_frenet.position.s, x0_frenet.position.t, x0_frenet.yaw, v0, 0.0, 0.0};
  ...
  auto sol_in = casadi::DMDict{{"X_optm_ref", X_optm_ref},
  ...
  for (int i = 0; i < num_step; i++) {
    const auto left_ref = traj.left_boundary_interpolation_function()(
        X_optm_ref(XIndex::PX, Slice()))[0];
    const auto right_ref = traj.right_boundary_interpolation_function()(
        X_optm_ref(XIndex::PX, Slice()))[0];
    const auto curvature_ref = traj.curvature_interpolation_function()(
        X_optm_ref(XIndex::PX, Slice()))[0];
    const auto vel_ref = traj.velocity_interpolation_function()(
        X_optm_ref(XIndex::PX, Slice()))[0];
    ...
    std::cout << "MPC Execution Time: " << duration.count() << "ms"
              << std::endl;
    if (mpc->solved()) {
    ...
    X_optm_out_global(Slice(XIndex::PX, XIndex::YAW + 1), Slice()) =
        f2g(X_optm_out(Slice(XIndex::PX, XIndex::YAW + 1), Slice()))[0];
...
TEST(RacingLMPCTest, MPCSolveTest) {
  const lmpc::Pose2D x0_pose2d{84.83, -112.67, -2.3532};
...
TEST(RacingLMPCTest, MPCSolveTestStartDeviated) {
  const lmpc::Pose2D x0_pose2d{85.4, -113.3, -2.3532};
  const double v0 = 10.0;
  const casadi_int num_step = 10;
  test_mpc(x0_pose2d, v0, num_step);
  SUCCEED();
