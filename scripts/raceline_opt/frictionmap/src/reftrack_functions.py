...
def load_reftrack(path2track: str) -> np.ndarray:
    """
    Documentation
...
def check_isclosed_refline(refline: np.ndarray) -> bool:
    ...
    dist_last2first = math.sqrt(
        (refline[-1, 0] - refline[0, 0]) ** 2 + (refline[-1, 1] - refline[0, 1]) ** 2
    )
...
def calc_trackboundaries(reftrack: np.ndarray) -> tuple:
    ...
    track_boundary_right = reftrack[:, :2] + refline_normvecs[:, :2] * np.expand_dims(
        reftrack[:, 2], axis=1
    )
    track_boundary_left = reftrack[:, :2] - refline_normvecs[:, :2] * np.expand_dims(
        reftrack[:, 3], axis=1
    )
...
def calc_refline_normvecs(refline: np.ndarray) -> np.ndarray:
    """
    Documentation
...
def plot_refline(reftrack: np.ndarray) -> None:
    ...
    for row in range(0, refline_normvecs.shape[0]):
        plt.plot(
            ...
            [
                reftrack[row, 1],
                reftrack[row, 1] + (-refline_normvecs[row, 1] * reftrack[row, 3]),
            ],
            "g",
        )
        ...
        plt.plot(
            ...
            [
                reftrack[row, 1],
                reftrack[row, 1] + (refline_normvecs[row, 1] * reftrack[row, 2]),
            ],
            "r",
        )
