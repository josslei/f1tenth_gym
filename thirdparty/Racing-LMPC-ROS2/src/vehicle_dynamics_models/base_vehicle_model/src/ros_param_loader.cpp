...
BaseVehicleModelConfig::SharedPtr load_parameters(rclcpp::Node *node) {
  ...
  auto front_tyre_config = std::make_shared<TyreConfig>(TyreConfig{
      declare_double("front_tyre.radius"), declare_double("front_tyre.width"),
      declare_double("front_tyre.mass"), declare_double("front_tyre.moi"),
  ...
  auto rear_tyre_config = std::make_shared<TyreConfig>(TyreConfig{
      declare_double("rear_tyre.radius"), declare_double("rear_tyre.width"),
      declare_double("rear_tyre.mass"), declare_double("rear_tyre.rot"),
  ...
  auto front_brake_config = std::make_shared<BrakeConfig>(
      BrakeConfig{declare_double("front_brake.max_brake"),
  ...
  auto rear_brake_config = std::make_shared<BrakeConfig>(
      BrakeConfig{declare_double("rear_brake.max_brake"),
  ...
  auto steer_config = std::make_shared<SteerConfig>(SteerConfig{
      declare_double("steer.max_steer_rate"), declare_double("steer.max_steer"),
      declare_double("steer.turn_left_bias")});

  auto chassis_config = std::make_shared<ChassisConfig>(ChassisConfig{
      declare_double("chassis.total_mass"),
      declare_double("chassis.sprung_mass"),
      declare_double("chassis.unsprung_mass"),
      declare_double("chassis.cg_ratio"), declare_double("chassis.cg_height"),
      declare_double("chassis.wheel_base"), declare_double("chassis.tw_f"),
      declare_double("chassis.tw_r"), declare_double("chassis.moi"),
      declare_double("chassis.b"), declare_double("chassis.fr")});

  auto aero_config = std::make_shared<AeroConfig>(AeroConfig{
      declare_double("aero.air_density"), declare_double("aero.drag_coeff"),
      declare_double("aero.frontal_area"), declare_double("aero.cl_f"),
      declare_double("aero.cl_r")});

  auto powertrain_config = std::make_shared<PowerTrainConfig>(PowerTrainConfig{
      lmpc::utils::Lookup3D{declare_vec("powertrain.rpm"),
  ...
  auto modeling_config = std::make_shared<ModelingConfig>(ModelingConfig{
      declare_bool("modeling.use_frenet"),
      integrator_type,
      declare_double("modeling.sample_throttle"),
  });

  return std::make_shared<BaseVehicleModelConfig>(BaseVehicleModelConfig{
      front_tyre_config, rear_tyre_config, front_brake_config,
      rear_brake_config, steer_config, chassis_config, aero_config,
      powertrain_config, modeling_config});
}
} // namespace base_vehicle_model
} // namespace vehicle_model
} // namespace lmpc
