...
size_t DoubleTrackPlanarModel::nx() const { return 6; }

size_t DoubleTrackPlanarModel::nu() const { return 3; }

void DoubleTrackPlanarModel::add_nlp_constraints(casadi::Opti &opti,
                                                 const casadi::MXDict &in) {
  // using casadi::MX;
  ...
  const auto k = base_config_->modeling_config->use_frenet
                     ? in.at("k")
                     : casadi::MX::sym("k", 1, 1);
  ...
  const auto &max_steer_rate =
      get_base_config()
          .steer_config->max_steer_rate; // times 2 for full left to full right

  // dynamics constraint
  auto xip1_temp = casadi::MX(xip1);
  xip1_temp(XIndex::YAW) = lmpc::utils::align_yaw<casadi::MX>(
      xip1_temp(XIndex::YAW), x(XIndex::YAW));
  if (base_config_->modeling_config->use_frenet) {
    xip1_temp(XIndex::PX) = lmpc::utils::align_abscissa<casadi::MX>(
        xip1_temp(XIndex::PX), x(XIndex::PX), in.at("track_length"));
  }
  // TODO(haoru): choose different integrator based on config
  const auto out1 =
      dynamics_gamma_y_({{"x", x}, {"u", u}, {"gamma_y", gamma_y}, {"k", k}});
  const auto k1 = out1.at("x_dot");
  const auto out2 = dynamics_gamma_y_(
      {{"x", x + t / 2.0 * k1}, {"u", u}, {"gamma_y", gamma_y}, {"k", k}});
  const auto k2 = out2.at("x_dot");
  const auto out3 = dynamics_gamma_y_(
      {{"x", x + t / 2.0 * k2}, {"u", u}, {"gamma_y", gamma_y}, {"k", k}});
  const auto k3 = out3.at("x_dot");
  const auto out4 = dynamics_gamma_y_(
      {{"x", x + t * k3}, {"u", u}, {"gamma_y", gamma_y}, {"k", k}});
  ...
  for (int i = 0; i < 4; i++) {
    opti.subject_to(pow(Fx_ij(i) / (mu * Fz_ij(i)), 2) +
                        pow(Fy_ij(i) / (mu * Fz_ij(i)), 2) <=
                    1);
  }

  // load transfer constraint
  opti.subject_to(gamma_y ==
                  hcog / (0.5 * (twf + twr)) *
                      (Fy_ij(TI::RL) + Fy_ij(TI::RR) +
                       (Fx_ij(TI::FL) + Fx_ij(TI::FR)) * sin(delta) +
...
void DoubleTrackPlanarModel::calc_lon_control(const casadi::DMDict &in,
                                              double &throttle,
...
void DoubleTrackPlanarModel::calc_lat_control(const casadi::DMDict &in,
...
void DoubleTrackPlanarModel::compile_dynamics() {
  ...
  const auto &kb_f =
      get_base_config().front_brake_config->bias; // front brake force bias
  ...
  const auto &lr =
      get_base_config().chassis_config->cg_ratio * l;       // cg to front axle
  ...
  const auto &fr =
      get_base_config().chassis_config->fr; // rolling resistance coefficient
  const auto &hcog =
      get_base_config().chassis_config->cg_height; // center of gravity height
  const auto &kroll_f = get_config().kroll_f; // front roll moment distribution
  const auto &cl_f =
      get_base_config().aero_config->cl_f; // downforce coefficient at front
  const auto &cl_r =
      get_base_config().aero_config->cl_r; // downforce coefficient at rear
  ...
  const auto &cd =
      get_base_config().aero_config->drag_coeff; // drag coefficient
  ...
  const auto &eps_f =
      tyre_f.pacejka_eps; // extended magic formula epsilon - front
  ...
  const auto &eps_r =
      tyre_r.pacejka_eps; // extended magic formula epsilon - rear

  // longitudinal tyre force Fx (eq. 4a, 4b)
  // TODO(haoru): consider differential
  const auto Fx_f =
      0.5 * kd_f * fd + 0.5 * kb_f * fb - 0.5 * fr * m * GRAVITY * lr / l;
  const auto Fx_fl = Fx_f;
  const auto Fx_fr = Fx_f;
  const auto Fx_r = 0.5 * (1 - kd_f) * fd + 0.5 * (1.0 - kb_f) * fb -
                    0.5 * fr * m * GRAVITY * lf / l;
  ...
  const auto Fz_f = 0.5 * m * GRAVITY * lr / (lf + lr) -
                    0.5 * hcog / (lf + lr) * m * ax +
                    0.25 * cl_f * rho * A * v_sq;
  const auto Fz_fl = Fz_f - kroll_f * gamma_y;
  const auto Fz_fr = Fz_f + kroll_f * gamma_y;
  const auto Fz_r = 0.5 * m * GRAVITY * lr / (lf + lr) +
                    0.5 * hcog / (lf + lr) * m * ax +
                    0.25 * cl_r * rho * A * v_sq;
  const auto Fz_rl = Fz_r - (1.0 - kroll_f) * gamma_y;
  const auto Fz_rr = Fz_r + (1.0 - kroll_f) * gamma_y;

  // tyre sideslip angles alpha (eq. 6a, 6b)
  const auto a_fl = delta - atan((lf * omega + v * sin(beta)) /
                                 (v * cos(beta) - 0.5 * twf * omega));
  const auto a_fr = delta - atan((lf * omega + v * sin(beta)) /
                                 (v * cos(beta) + 0.5 * twf * omega));
  const auto a_rl =
      atan((lr * omega - v * sin(beta)) / (v * cos(beta) - 0.5 * twr * omega));
  const auto a_rr =
      atan((lr * omega - v * sin(beta)) / (v * cos(beta) + 0.5 * twr * omega));

  // lateral tyre force Fy (eq. 5)
  const auto Fy_fl =
      mu * Fz_fl * (1.0 + eps_f * Fz_fl / Fz0_f) *
      sin(Cf * atan(Bf * a_fl - Ef * (Bf * a_fl - atan(Bf * a_fl))));
  const auto Fy_fr =
      mu * Fz_fr * (1.0 + eps_f * Fz_fr / Fz0_f) *
      sin(Cf * atan(Bf * a_fr - Ef * (Bf * a_fr - atan(Bf * a_fr))));
  const auto Fy_rl =
      mu * Fz_rl * (1.0 + eps_r * Fz_rl / Fz0_r) *
      sin(Cr * atan(Br * a_rl - Er * (Br * a_rl - atan(Br * a_rl))));
  const auto Fy_rr =
      mu * Fz_rr * (1.0 + eps_r * Fz_rr / Fz0_r) *
      sin(Cr * atan(Br * a_rr - Er * (Br * a_rr - atan(Br * a_rr))));

  // dynamics (eq. 3a, 3b, 3c)
  const auto v_dot =
      1.0 / m *
      ((Fx_rl + Fx_rr) * cos(beta) + (Fx_fl + Fx_fr) * cos(delta - beta) +
       (Fy_rl + Fy_rr) * sin(beta) - (Fy_fl + Fy_fr) * sin(delta - beta) -
       0.5 * cd * rho * A * v_sq * cos(beta));
  const auto beta_dot =
      -omega +
      1.0 / (m * v) *
          (-(Fx_rl + Fx_rr) * sin(beta) + (Fx_fl + Fx_fr) * sin(delta - beta) +
           (Fy_rl + Fy_rr) * cos(beta) + (Fy_fl + Fy_fr) * cos(delta - beta) +
           0.5 * cd * rho * A * v_sq * sin(beta));
  const auto omega_dot =
      1.0 / Jzz *
      ((Fx_rr - Fx_rl) * twr / 2 - (Fy_rl + Fy_rr) * lr +
       ((Fx_fr - Fx_fl) * cos(delta) + (Fy_fl - Fy_fr) * sin(delta)) * twf /
           2.0 +
  ...
  dynamics_gamma_y_ =
      casadi::Function("double_track_planar_model", {x, u, gamma_y, k},
                       {x_dot, Fx_ij, Fy_ij, Fz_ij}, {"x", "u", "gamma_y", "k"},
  ...
  dynamics_jacobian_ = casadi::Function(
      "double_track_planar_model_jacobian", {x, u, gamma_y, k}, {Ac, Bc, B2c},
      {"x", "u", "gamma_y", "k"}, {"A", "B", "B2"});

  using TI = utils::TyreIndex;
  const casadi::SXDict dyn_in = {
      {"x", x}, {"u", u}, {"k", k}, {"gamma_y", gamma_y}};
  ...
  const auto res =
      gamma_y - hcog / (0.5 * (twf + twr)) *
                    (Fy_ij_fwd(TI::RL) + Fy_ij_fwd(TI::RR) +
                     (Fx_ij_fwd(TI::FL) + Fx_ij_fwd(TI::FR)) * sin(delta) +
                     (Fy_ij_fwd(TI::FL) + Fy_ij_fwd(TI::FR)) * cos(delta));
  auto g = casadi::Function("g", {gamma_y}, {res});
  auto lateral_load_transfer_ =
      casadi::rootfinder("G", "newton", g, {{"error_on_fail", false}});

  const auto gamma_y_solve =
      casadi::DM(lateral_load_transfer_(casadi::DM{0.0}));
  for (auto &var : dyn_out) {
    dyn_out[var.first] =
        casadi::DM(casadi::SX::substitute(var.second, gamma_y, gamma_y_solve));
  }
  dynamics_ = casadi::Function(
      "double_track_planar_model_forward_dynamics", {x, u, k},
      {dyn_out.at("x_dot"), dyn_out.at("Fx_ij"), dyn_out.at("Fy_ij"),
       dyn_out.at("Fz_ij"), gamma_y_solve},
      {"x", "u", "k"}, {"x_dot", "Fx_ij", "Fy_ij", "Fz_ij", "gamma_y"});

  // discretize dynamics
  SX xip1;
  const auto &integrator_type =
      get_base_config().modeling_config->integrator_type;
  ...
  discrete_dynamics_ = casadi::Function(
      "double_track_planar_model_discrete_dynamics", {x, u, k, dt},
      {xip1, Fx_ij, Fy_ij, Fz_ij}, {"x", "u", "k", "dt"},
  ...
  discrete_dynamics_jacobian_ = casadi::Function(
      "double_track_planar_model_discrete_dynamics_jacobian", {x, u, k, dt},
      {Ad, Bd}, {"x", "u", "k", "dt"}, {"A", "B"});
}
} // namespace double_track_planar_model
} // namespace vehicle_model
