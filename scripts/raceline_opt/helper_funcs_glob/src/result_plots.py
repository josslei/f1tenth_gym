...
def result_plots(
    ...
    if plot_opts["raceline"]:
        ...
        normvec_normalized_opt = (
            trajectory_planning_helpers.calc_normal_vectors.calc_normal_vectors(  # pyright: ignore[reportAttributeAccessIssue]
                trajectory[:, 3]
            )
        )
    ...
    if plot_opts["racetraj_vel_3d"]:
        ...
        _get_proj_orig = ax.get_proj  # pyright: ignore[reportAttributeAccessIssue]
        ax.get_proj = lambda: np.dot(
            _get_proj_orig(), np.diag([scale_x, scale_y, scale_z, 1.0])
        )
