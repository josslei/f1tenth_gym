...
// state vectors; lmpc_controller.hpp re-exports these into the lmpc
// namespace rather than redefining them.
...
public:
  ...
  // u = [a, delta] at the current stage; u_prev = [a, delta] at the
  // previous stage -- carried because the FHOCP (DESIGN.md SS3/SS4)
  // already threads u_{t-1} through every stage for the control-rate cost,
  // so rate-dependent models (e.g. ExtendedKinematicDynamics) can reuse it
  // instead of adding a new state. Models that don't need it (e.g.
  // GymDynamics) simply ignore it.
...
inline casadi::SX frenet_pose_kinematics(
  const casadi::SX & x_vel,
  ...
  const casadi::SX & kappa)
{
  ...
  const casadi::SX s_dot = (vx * cos(epsi) - vy * sin(epsi)) / (1.0 - ey * kappa);
...
inline casadi::SX full_state_dynamics(
