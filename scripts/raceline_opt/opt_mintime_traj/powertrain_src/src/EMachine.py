# pyright: reportOptionalOperand=false, reportOperatorIssue=false, reportOptionalMemberAccess=false
# (suppressed: None-initialized slots populated by initialize() at runtime)
...
class EMachineModel:
    ...
    def __init__(self, pwr_pars: dict):
        """
        Python version: 3.5
    ...
    def initialize(self):
        ...
        self.temp_mot_n = ca.SX.sym("temp_mot_n")  # pyright: ignore[reportArgumentType]
    ...
    def get_states(self, f_drive: ca.SX, v: ca.SX):
        """
        Python version: 3.5
        Created by: Thomas Herrmann
        Created on: 01.04.2020
        ...
        self.i_eff = (
            f_drive
            * self.pars["r_wheel"]
            / self.pars["MotorConstant"]
            / self.pars["transmission"]
        ) / self.pars["N_machines"]

        # Rotational speed electric machine > speed wheel; v = 2 pi f r [rpm]
        self.omega_machine = (
            v / (2 * np.pi * self.pars["r_wheel"]) * self.pars["transmission"] * 60
        )

    def get_increment(self, sf: ca.SX, temp_cool_12: ca.SX, temp_cool_13: ca.SX):
        """
        Python version: 3.5
        Created by: Thomas Herrmann
        Created on: 01.04.2020
        ...
        self.dtemp = sf * (
            (
                self.p_loss_total * 1000
                - (self.temp_mot - (temp_cool_12 + temp_cool_13) / 2) / self.r_machine
            )
            / (self.pars["C_therm_machine"])
        )

    def get_loss(self, p_wheel: ca.SX):
        """
        Python version: 3.5
        Created by: Thomas Herrmann
        Created on: 01.04.2020
        ...
        if self.pars["simple_loss"]:
            ...
            p_loss_copper = (
                (
                    (((temp_mot - 20) * self.pars["C_TempCopper"]) + 1)
                    * self.pars["R_Phase"]
                )
                * (i_eff**2)
                * (3 / 2)
    ...
    def get_thermal_resistance(self):
        ...
        A_cool_machine = (
            2
            * np.pi
            * self.pars["r_stator_ext"]
            * self.pars["l_machine"]
            * self.pars["A_cool_inflate_machine"]
        )

        # Thermal conduction stator [K/W]
        r_cond_stator = np.log(
            self.pars["r_stator_ext"] / self.pars["r_stator_int"]
    ...
    def ini_nlp_state(self, x: ca.SX, u: ca.SX):
        """
        Python version: 3.5
        Created by: Thomas Herrmann
        Created on: 01.04.2020
        ...
        if self.pars["simple_loss"]:
            self.f_nlp = ca.Function(
                "f_nlp",
                [x, u],
                [self.p_loss_total, self.p_input],
                ["x", "u"],
                ["p_loss_total", "p_input"],
            )
        else:
            self.f_nlp = ca.Function(
                "f_nlp",
    ...
    def extract_sol(self, w: ca.SX, sol_states: ca.DM):
        """
        Python version: 3.5
        Created by: Thomas Herrmann
        Created on: 01.04.2020
        ...
        if self.pars["simple_loss"]:
            self.f_sol = ca.Function(
                "f_sol", [w], [self.p_losses_opt], ["w"], ["p_losses_opt"]
            )

            # Overwrite lists with optimized numeric values
            p_losses_opt = self.f_sol(sol_states)

            self.p_loss_total = p_losses_opt[0::2]  # pyright: ignore[reportOptionalSubscript]
            self.p_input = p_losses_opt[1::2]  # pyright: ignore[reportOptionalSubscript]
            ...
            self.p_loss_total = p_losses_opt[0::5]  # pyright: ignore[reportOptionalSubscript]
            self.p_loss_copper = p_losses_opt[1::5]  # pyright: ignore[reportOptionalSubscript]
            self.p_loss_stator_iron = p_losses_opt[2::5]  # pyright: ignore[reportOptionalSubscript]
            self.p_loss_rotor = p_losses_opt[3::5]  # pyright: ignore[reportOptionalSubscript]
            self.i_eff = p_losses_opt[4::5]  # pyright: ignore[reportOptionalSubscript]
