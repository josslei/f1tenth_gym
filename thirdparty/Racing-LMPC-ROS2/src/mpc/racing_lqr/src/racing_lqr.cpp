...
RacingLQR::RacingLQR(RacingLQRConfig::SharedPtr mpc_config,
    ...
    : config_(mpc_config), model_(model),
      ...
      rk4_(utils::rk4_function(model_->nx(), model_->nu(),
                               model_->dynamics())) {}

const RacingLQRConfig &RacingLQR::get_config() const { return *config_.get(); }
...
void RacingLQR::solve(const casadi::DMDict &in, casadi::DMDict &out) {
  ...
  auto K =
      casadi::DMVector(config_->N - 1, DM::zeros(model_->nu(), model_->nx()));
  auto As =
      casadi::DMVector(config_->N - 1, DM::zeros(model_->nx(), model_->nx()));
  auto Bs =
      casadi::DMVector(config_->N - 1, DM::zeros(model_->nx(), model_->nu()));
  for (int k = config_->N - 2; k >= 0; k--) {
    ...
    // Ricatti
    K[k] = DM::solve(config_->R + DM::mtimes({Bs[k].T(), P[k + 1], Bs[k]}),
                     DM::mtimes({Bs[k].T(), P[k + 1], As[k]}));
    P[k] = config_->Q +
           DM::mtimes({As[k].T(), P[k + 1], As[k] - DM::mtimes(Bs[k], K[k])});
  }
  ...
  for (size_t k = 0; k < config_->N - 1; k++) {
    U_optm(Slice(), k) =
        U_ref(Slice(), k) -
        DM::mtimes(K[k], X_optm(Slice(), k) - X_ref(Slice(), k));
    // TODO(haoru): add frenet support
    X_optm(Slice(), k + 1) = rk4_(casadi::DMDict{{"x", X_optm(Slice(), k)},
                                                 {"u", U_optm(Slice(), k)},
                                                 {"dt", config_->dt},
                                                 {"k", 0.0}})
                                 .at("xip1");
...
SingleTrackPlanarModel &RacingLQR::get_model() { return *model_; }
} // namespace racing_lqr
} // namespace mpc
} // namespace lmpc
