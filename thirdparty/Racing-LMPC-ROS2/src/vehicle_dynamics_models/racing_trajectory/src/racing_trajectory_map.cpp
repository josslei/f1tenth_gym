  ...
  for (const auto &entry :
    ...
    if (!std::regex_match(file_name, match, pattern)) {
      throw std::runtime_error(
          "File does not conform to the required format [number_name.txt]: " +
          file_name);
    }

    int number = std::stoi(match[1]);
    if (trajectories_.find(number) != trajectories_.end()) {
      throw std::runtime_error("Duplicate trajectory number found: " +
    ...
    std::cout << "Loaded trajectory " << number << " from " << traj_path
              << ". ";
    std::cout << "Length: " << trajectories_[number]->total_length() << " m."
              << std::endl;
  }
}

RacingTrajectory::SharedPtr
RacingTrajectoryMap::get_trajectory(const int &index) {
