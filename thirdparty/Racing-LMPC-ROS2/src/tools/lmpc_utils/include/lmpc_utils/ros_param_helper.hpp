...
T declare_parameter(rclcpp::Node *node, const char *name) {
  try {
    ...
    RCLCPP_FATAL(node->get_logger(),
                 "Parameter \"%s\" value does not exsist or is invalid: %s",
                 name, e.what());
    throw e;
  } catch (rclcpp::exceptions::InvalidParametersException &e) {
    RCLCPP_FATAL(node->get_logger(), "Parameter \"%s\" name is invalid: %s",
                 name, e.what());
    throw e;
  } catch (rclcpp::exceptions::InvalidParameterTypeException &e) {
    RCLCPP_FATAL(node->get_logger(), "Parameter \"%s\" type mismatch: %s", name,
                 e.what());
    throw e;
  } catch (rclcpp::exceptions::ParameterAlreadyDeclaredException &e) {
    RCLCPP_FATAL(node->get_logger(), "Parameter \"%s\" is already declared: %s",
                 name, e.what());
    throw e;
  }
}
