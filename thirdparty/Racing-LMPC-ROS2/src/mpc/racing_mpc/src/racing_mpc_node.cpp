...
RacingMPCNode::RacingMPCNode(const rclcpp::NodeOptions &options)
    : rclcpp::Node("racing_mpc_node", options),
      ...
      delay_step_(
          utils::declare_parameter<int>(this, "racing_mpc_node.delay_step")),
      ...
      model_(vehicle_model::vehicle_model_factory::load_vehicle_model(
          utils::declare_parameter<std::string>(
              this, "racing_mpc_node.vehicle_model_name"),
          this)),
      mpc_(std::make_shared<RacingMPC>(config_, model_, false)),
      profiler_(std::make_unique<lmpc::utils::CycleProfiler<double>>(10)),
      profiler_iter_count_(
          std::make_unique<lmpc::utils::CycleProfiler<double>>(10)),
      speed_scale_(utils::declare_parameter<double>(
          this, "racing_mpc_node.velocity_profile_scale")),
  ...
  vehicle_actuation_msg_ =
      std::make_shared<mpclab_msgs::msg::VehicleActuationMsg>();
  ...
  const auto k =
      track_->curvature_interpolation_function()(x_sym(XIndex::PX))[0];
  const auto xip1 = model_
                        ->discrete_dynamics()(casadi::MXDict{
                            {"x", x_sym}, {"u", u_sym}, {"k", k}, {"dt", dt_}})
                        .at("xip1");
  discrete_dynamics_ =
      casadi::Function("discrete_dynamics", {x_sym, u_sym}, {xip1});

  // initialize the publishers
  vehicle_actuation_pub_ =
      this->create_publisher<mpclab_msgs::msg::VehicleActuationMsg>(
          "vehicle_actuation", 1);
  mpc_vis_pub_ =
      this->create_publisher<nav_msgs::msg::Path>("mpc_visualization", 1);
  ref_vis_pub_ =
      this->create_publisher<nav_msgs::msg::Path>("ref_visualization", 1);
  diagnostics_pub_ =
      this->create_publisher<diagnostic_msgs::msg::DiagnosticArray>(
          "diagnostics", 1);
  ss_vis_pub_ = this->create_publisher<visualization_msgs::msg::MarkerArray>(
      "ss_visualization", 1);
  mpc_telemetry_pub_ =
      this->create_publisher<lmpc_msgs::msg::MPCTelemetry>("mpc_telemetry", 1);

  // initialize the subscribers
  // state subscription is on a separate callback group
  // Together with the multi-threaded executor (defined in main()), this allows
  // the state subscription to be processed in parallel with the mpc computation
  // in continuous mode
  ...
  vehicle_state_sub_ =
      this->create_subscription<mpclab_msgs::msg::VehicleStateMsg>(
          "vehicle_state", 1,
          std::bind(&RacingMPCNode::on_new_state, this, std::placeholders::_1),
          state_sub_options);

  trajectory_command_callback_group_ =
      this->create_callback_group(rclcpp::CallbackGroupType::MutuallyExclusive);
  rclcpp::SubscriptionOptions trajectory_command_sub_options;
  trajectory_command_sub_options.callback_group =
      trajectory_command_callback_group_;
  trajectory_command_sub_ =
      this->create_subscription<lmpc_msgs::msg::TrajectoryCommand>(
          "lmpc_trajectory_command", 1,
          std::bind(&RacingMPCNode::on_new_trajectory_command, this,
                    std::placeholders::_1),
          trajectory_command_sub_options);

  // initialize the parameter callback
  callback_handle_ = add_on_set_parameters_callback(std::bind(
      &RacingMPCNode::on_set_parameters, this, std::placeholders::_1));

  if (config_->step_mode == RacingMPCStepMode::CONTINUOUS) {
    // initialize the timers
    step_timer_ =
        this->create_wall_timer(std::chrono::duration<double>(dt_),
                                std::bind(&RacingMPCNode::on_step_timer, this));
  }
}

void RacingMPCNode::on_new_state(
...
void RacingMPCNode::on_step_timer() {
  ...
  auto x_ic_base = DM{p.s, p.x_tran, p.e_psi, v.v_long, v.v_tran, w.w_psi};
  const Pose2D current_global_pose{
      {vehicle_state_msg_->x.x, vehicle_state_msg_->x.y},
  ...
  const auto x_ic = model_
                        ->from_base_state()(
                            casadi::DMDict{{"x", x_ic_base}, {"u", u_ic_base}})
                        .at("x_out");
  const auto u_ic = model_
                        ->from_base_control()(
                            casadi::DMDict{{"x", x_ic_base}, {"u", u_ic_base}})
                        .at("u_out");
  // current input
  ...
  if (!mpc_full_->solved()) {
    ...
    for (int i = 1; i < N; i++) {
      last_x_(Slice(), i) = discrete_dynamics_(casadi::DMVector{
          last_x_(Slice(), i - 1), last_u_(Slice(), i - 1)})[0];
    }
    ...
    if (config_->step_mode == RacingMPCStepMode::CONTINUOUS) {
      sol_in_["x_ic"] =
          discrete_dynamics_(casadi::DMVector{x_ic, last_u_(Slice(), 0)})[0];
    ...
    last_x_ = DM::horzcat(
        {last_x_(Slice(), Slice(1, N)), DM::zeros(model_->nx(), 1)});
    last_u_ = DM::horzcat(
        {last_u_(Slice(), Slice(1, N - 1)), last_u_(Slice(), Slice(N - 2))});
    last_du_ = DM::horzcat(
        {last_du_(Slice(), Slice(1, N - 1)), DM::zeros(model_->nu(), 1)});
    last_x_(Slice(), -1) = discrete_dynamics_(
        casadi::DMVector{last_x_(Slice(), -2), last_u_(Slice(), -1)})[0];
    sol_in_["X_ref"] = last_x_;
    sol_in_["U_ref"] = last_u_;
    sol_in_["X_optm_ref"] = last_x_;
    sol_in_["U_optm_ref"] = last_u_;
  ...
  const auto curvature_ref =
      track_->curvature_interpolation_function()(abscissa)[0];
  ...
  for (int i = 0; i < config_->N; i++) {
    ...
    if (ref_speed > 0.0) {
      const auto ref_speed_clipped =
          std::clamp(ref_speed, current_speed - config_->max_vel_ref_diff,
                     current_speed + config_->max_vel_ref_diff);
  ...
  if (!mpc_full_->solved()) {
    ...
    if (mpc_full_->solved()) {
      RCLCPP_INFO(this->get_logger(),
                  "Solved the first time with full dynamics.");
    } else {
      RCLCPP_FATAL(this->get_logger(),
                   "Failed to solve the first time with full dynamics.");
    }
    return;
  }
  ...
  if (!jitted) {
    RCLCPP_INFO(this->get_logger(),
                "Using the first solve to execute just-in-time compilation.");
  }
  ...
  if (!jitted) {
    ...
    jitted = true;
    RCLCPP_INFO(this->get_logger(),
                "JIT is done. Discarding the first solve...");
    return;
  }
  ...
  x_ref_global(Slice(XIndex::PX, XIndex::YAW + 1), Slice()) = f2g_(
      sol_in_.at("X_optm_ref")(Slice(XIndex::PX, XIndex::YAW + 1), Slice()))[0];

  const auto mpc_solve_duration =
      std::chrono::system_clock::now() - mpc_solve_start;
  ...
  if (stats.count("iter_count")) {
    profiler_iter_count_->add_cycle_stats(
        static_cast<double>(stats.at("iter_count")));
  }
  profile_step_count++;
  ...
  if (profile_step_count % profiler_->capacity() == 0) {
    ...
    diagnostics_msg.status.push_back(profiler_->profile().to_diagnostic_status(
  ...
  const auto last_u_base = model_
                               ->to_base_control()(casadi::DMDict{
                                   {"x", last_x_(Slice(), delay_step_)},
                                   {"u", last_u_(Slice(), delay_step_)}})
                               .at("u_out");
  const auto u_vec = last_u_base.get_elements();
  // std::cout << "x: " << last_x_(Slice(), 0) << std::endl;
  // std::cout << "u: " << last_u_(Slice(), 0) << std::endl;
  // std::cout << "xip1: "
  //   << discrete_dynamics_(casadi::DMVector{last_x_(Slice(), 0),
  //   last_u_(Slice(), 0)})[0]
  //   << std::endl << std::endl;
  ...
  for (int i = 0; i < N; i++) {
    ...
    pose.pose.orientation =
        tf2::toMsg(utils::TransformHelper::quaternion_from_heading(
  ...
  if (sol_out.count("ss_x")) {
    ...
    const auto scale =
        model_->get_base_config().chassis_config->wheel_base / 10.0;
    ...
    for (casadi_int i = 0; i < ss_X.size2(); i++) {
      ...
      const auto ss_x_global =
          track_->frenet_to_global_function()(ss_x_frenet)[0].get_elements();
    ...
    if (parameter.get_name() == "racing_mpc_node.velocity_profile_scale") {
      ...
      if (speed_scale < 0.0) {
        RCLCPP_WARN(this->get_logger(),
                    "Invalid velocity scale %f, must be non-negative",
                    speed_scale);
        return result;
      }

      set_speed_scale(speed_scale);
      RCLCPP_INFO(this->get_logger(), "Set velocity scale to %f", speed_scale_);
      result.successful = true;
    } else {
      RCLCPP_WARN(this->get_logger(), "Cannot set parameter %s",
...
void RacingMPCNode::change_trajectory(const int &traj_idx) {
  ...
  if (new_traj) {
    ...
    if (vehicle_state_msg_) {
      ...
      if (mpc_->solved()) {
        for (int i = 0; i < config_->N; i++) {
          ...
          const FrenetPose2D old_frenet_pose{{xi[XIndex::PX], xi[XIndex::PY]},
                                             xi[XIndex::YAW]};
    ...
    RCLCPP_INFO(this->get_logger(), "Changed trajectory to %d.", traj_idx_);
  }
}
...
void RacingMPCNode::set_speed_limit(const double &speed_limit) {
  ...
  speed_limit_lock.unlock();
  RCLCPP_INFO(this->get_logger(), "Set speed limit to %f m/s.", speed_limit_);
}
...
void RacingMPCNode::set_speed_scale(const double &speed_scale) {
  ...
  if (speed_scale > 1.0 || speed_scale <= 0.0) {
    RCLCPP_WARN(this->get_logger(),
                "Invalid velocity scale %f, must be between (0.0-1.0]. "
                "Resetting to 20%%.",
                speed_scale);
  } else {
    scale = speed_scale;
    RCLCPP_INFO(this->get_logger(), "Set velocity scale to %f", speed_scale);
  }
