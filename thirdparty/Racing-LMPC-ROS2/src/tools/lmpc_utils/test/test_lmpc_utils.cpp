...
TEST(LmpcUtilsTest, RosParamHelperTest) {
  ...
  options.arguments({"--ros-args", "-p", "test_double:=1.0", "-p",
                     "test_int:=1", "-p", "test_vec:=[3.14,2.718]"});
  auto test_node = rclcpp::Node("test_lmpc_utils_node", options);

  EXPECT_DOUBLE_EQ(
      lmpc::utils::declare_parameter<double>(&test_node, "test_double"), 1.0);
  EXPECT_EQ(lmpc::utils::declare_parameter<int64_t>(&test_node, "test_int"), 1);
  EXPECT_NO_THROW(lmpc::utils::declare_parameter<std::vector<double>>(
      &test_node, "test_vec"));

  EXPECT_THROW(
      lmpc::utils::declare_parameter<double>(&test_node, "test_double"),
  ...
  EXPECT_ANY_THROW(lmpc::utils::declare_parameter<double>(
      &test_node, "test_name_invalid!@#"));
