...
using lmpc::vehicle_model::single_track_planar_model::
...
public:
  const char *what() {
    return "Call EKFStateEstimator::initialize() before making any observation "
           "updates.";
  }
};
...
public:
  const char *what() {
    return "Changes to observations are not allowed after the filter is "
           "initialized.";
  }
};
...
public:
  explicit ObservationNameAlreadyExistsException(const char *name) {
    msg_ = "The observation name \"" + std::string(name) +
...
public:
  ...
  explicit EKFStateEstimator(EKFStateEstimatorConfig::SharedPtr ekf_config,
   ...
   * @param h observation function taking nx x 1 state and outputs nz * 1
   * observation.
   *
   * @throws EKFAlreadyInitializedException if the filter is already
   * initialized.
   * @throws ObservationNameAlreadyExistsException if the observation name is
   * already taken.
   */
  void register_observation(const std::string &name, const casadi_int &nz,
                            casadi::Function &h);

  /**
   * @brief Call this after all observations are registered and before calling
   * any filter updates. No further changes to the observations are allowed
   * afterwards. Re-initialization is also allowed through this function, which
   * will reset state estimate and covariance estimate to the initial value in
   * config.
   *
   ...
   *
   * @param name observation name at registration. leave empty to carry a pure
   * prediction.
   ...
   *    no update is carried out if this is earlier than the latest time of the
   * filter.
   * @param out output:
   ...
   *
   * @throws EKFUninitializedException if calling before the filter is
   * initialized.
   * @throws ObservationNameNotFoundException if the observation name is not
   * registered.
   */
  void update_observation(const StrOpt &name, const casadi::DMDict &in,
                          casadi::DMDict &out);
...
protected:
  ...
  SliceDict
      slices_; // store how to slice the Kalman gain for a certain observation
