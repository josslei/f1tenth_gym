...
SSTrajectory::SSTrajectory(const casadi::DM &x, const casadi::DM &u,
...
SSResult SSTrajectory::query(const SSQuery &query) const {
  ...
  tree_.find_closest_waypoint_indices(static_cast<double>(query.x(0)),
...
std::vector<RegResult> SSTrajectory::query(const RegQuery &query) const {
  ...
  for (casadi_int i = 0;
       i < static_cast<casadi_int>(query.reg_out_state_idxs.size()); i++) {
    auto &result = results.emplace_back();
    if (query.reg_out_state_idxs[i].size() != 1) {
      throw std::invalid_argument(
          "Only one state variable is supported in every regression");
    }

    // slice the x matrix to get the regression input
    // the last x is not considered since no xip1 is available for it
    casadi::DM x_data =
        lap_.x(query.reg_in_state_idxs[i], casadi::Slice(0, -1));
    casadi::DM x_full_data = lap_.x(casadi::Slice(), casadi::Slice(0, -1));
    casadi::DM u_data =
        lap_.u(query.reg_in_control_idxs[i], casadi::Slice(0, -1));
    casadi::DM u_full_data = lap_.u(casadi::Slice(), casadi::Slice(0, -1));
    casadi::DM xip1_data =
        lap_.x(query.reg_out_state_idxs[i],
               casadi::Slice(1, std::numeric_limits<casadi_int>::max()));
    ...
    result.dists = casadi::DM::sqrt(casadi::DM::sum1(casadi::DM::pow(
    ...
    x_full_data = x_full_data(casadi::Slice(), mask);
    u_data = u_data(casadi::Slice(), mask);
    u_full_data = u_full_data(casadi::Slice(), mask);
    ...
    Eigen::VectorXi idx =
        Eigen::VectorXi::LinSpaced(dist_vec.size(), 0, dist_vec.size() - 1);
    std::sort(idx.data(), idx.data() + idx.size(),
              [&dist_eigen](size_t i1, size_t i2) {
                return dist_eigen(i1) < dist_eigen(i2);
              });
    // convert idx to std vector
    std::vector<casadi_int> idx_vec(idx.data(), idx.data() + idx.size());
    if (idx_vec.size() > static_cast<size_t>(query.max_num_per_lap)) {
      idx_vec.resize(query.max_num_per_lap);
    }
    // slice the data according to the sorted indices
    result.xs = x_data(casadi::Slice(), idx_vec);
    result.xs_full = x_full_data(casadi::Slice(), idx_vec);
    result.us = u_data(casadi::Slice(), idx_vec);
    result.us_full = u_full_data(casadi::Slice(), idx_vec);
    result.xip1s = xip1_data(casadi::Slice(), idx_vec);
    result.ks = k_data(casadi::Slice(), idx_vec);
    result.dts = dt_data(casadi::Slice(), idx_vec);
    result.dists = result.dists(idx_vec);
  }
  return results;
}

SSTrajectoryData
SSTrajectory::process_lap_data(const casadi::DM &x, const casadi::DM &u,
                               const casadi::DM &k, const casadi::DM &t,
                               const double &total_length) const {
  SSTrajectoryData data;
  data.dt = t(casadi::Slice(),
              casadi::Slice(1, std::numeric_limits<casadi_int>::max())) -
            t(casadi::Slice(), casadi::Slice(0, -1));
  data.dt = casadi::DM::horzcat({data.dt, data.dt(casadi::Slice(), -1)});
  const auto lap_time = t(casadi::Slice(), -1) - t(casadi::Slice(), 0) +
                        data.dt(casadi::Slice(), -1);
  const auto J = t(casadi::Slice(), -1) + data.dt(casadi::Slice(), -1) - t;
  ...
  data.u = u;
  data.k = k;
  data.J = casadi::DM::horzcat({J + lap_time, J, J - lap_time});
  data.x = x;
  return data;
}
...
void SafeSetManager::add_lap(const casadi::DM &x, const casadi::DM &u,
                             const casadi::DM &k, const casadi::DM &t,
                             const double &total_length) {
...
SSResult SafeSetManager::query(const SSQuery &query) {
  ...
  for (auto it = laps_.rbegin();
       num_total < query.max_num_total && it != laps_.rend(); ++it) {
...
RegResult SafeSetManager::query(const RegQuery &query) {
  ...
  std::shared_lock<std::shared_mutex> lock(mutex_);
#if defined(__cpp_lib_execution) && !defined(_LIBCPP_VERSION)
  std::transform(std::execution::par_unseq, laps_.begin(), laps_.end(),
                 results.begin(),
                 [&query](const auto &lap) { return lap->query(query); });
#else
  std::transform(laps_.begin(), laps_.end(), results.begin(),
                 [&query](const auto &lap) { return lap->query(query); });
#endif
  lock.unlock();
  ...
  for (casadi_int i = 0;
    ...
    for (const auto &lap_result : results) {
      ...
      xs_full = casadi::DM::horzcat({xs_full, lap_result[i].xs_full});
      us_full = casadi::DM::horzcat({us_full, lap_result[i].us_full});
    ...
    if (xs.size2() > query.max_num_total) {
      xs = xs(casadi::Slice(), casadi::Slice(0, query.max_num_total));
      us = us(casadi::Slice(), casadi::Slice(0, query.max_num_total));
      xs_full = xs_full(casadi::Slice(), casadi::Slice(0, query.max_num_total));
      us_full = us_full(casadi::Slice(), casadi::Slice(0, query.max_num_total));
      xip1s = xip1s(casadi::Slice(), casadi::Slice(0, query.max_num_total));
      dists = dists(casadi::Slice(0, query.max_num_total));
      ks = ks(casadi::Slice(), casadi::Slice(0, query.max_num_total));
      dts = dts(casadi::Slice(), casadi::Slice(0, query.max_num_total));
    }
    // predict the next state
    const auto xip1s_pred =
        query.f
            .map(xs.size2())(casadi::DMDict{
                {"x", xs_full}, {"u", us_full}, {"k", ks}, {"dt", dts}})
            .at("xip1");
    // compute Epanechnikov kernel weights
    const auto W =
        0.75 / query.dist_max *
        casadi::DM::pow(1 - casadi::DM::pow(dists / query.dist_max, 2), 2);
    // build the local linear system Theta Z = Y with homogeneous coordinates
    const auto Z_xu = casadi::DM::horzcat({xs.T(), us.T()});
    const auto Y = xip1s.T() -
                   xip1s_pred(query.reg_out_state_idxs[i], casadi::Slice()).T();
    const auto Z =
        casadi::DM::horzcat({Z_xu, casadi::DM::ones(Z_xu.size1(), 1)});
    const auto W_col = casadi::DM::reshape(W, W.numel(), 1);
    const auto weighted_Z = Z * casadi::DM::repmat(W_col, 1, Z.size2());
    const auto Q = casadi::DM::mtimes(Z.T(), weighted_Z) +
                   1e-3 * casadi::DM::eye(Z.size2());
    const auto rhs = casadi::DM::mtimes(Z.T(), Y * W_col);
    // solve the weighted ridge system for the local affine error coefficients
    const auto Theta = casadi::DM::solve(Q, rhs);
    const auto dA =
        Theta(casadi::Slice(0, static_cast<casadi_int>(
                                   query.reg_in_state_idxs[i].size())))
            .T();
    const auto dB = Theta(casadi::Slice(static_cast<casadi_int>(
                                            query.reg_in_state_idxs[i].size()),
                                        -1))
                        .T();
    const auto dC = Theta(-1);
...
SafeSetRecorder::SafeSetRecorder(SafeSetManager &manager, const bool &to_file,
                                 const std::string &file_prefix)
    : manager_(manager), last_x_valid_(false), initialized_(false),
      to_file_(to_file), file_prefix_(file_prefix), lap_count_(0) {}

void SafeSetRecorder::load(const std::vector<std::string> &from_files,
                           const double &total_length) {
...
void SafeSetRecorder::step(const casadi::DM &x, const casadi::DM &u,
                           const casadi::DM &k, const casadi::DM &t,
                           const double &total_length) {
  if (!last_x_valid_) {
    last_x_ = x;
  ...
  if (px_last - px > 0.5 * total_length) {
    // new lap
    if (initialized_) {
      std::cout << "Lap " << lap_count_ << " completed. Adding to safe set."
                << std::endl;
      std::cout << "Lap " << lap_count_ << " ave speed: "
                << static_cast<double>(total_length / (t - last_t_(0)))
                << " m/s, time: " << (t - last_t_(0)) << " s." << std::endl;
      manager_.add_lap(last_x_, last_u_, last_k_, last_t_, total_length);
      if (to_file_) {
        const auto filename =
            file_prefix_ + "lap_" + std::to_string(lap_count_);
