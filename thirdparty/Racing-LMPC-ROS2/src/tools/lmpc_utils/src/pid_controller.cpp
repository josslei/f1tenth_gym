...
bool PidController::try_update_param(std::string const &name,
                                     rclcpp::Parameter const &param) {
  if (param.get_type() != rclcpp::ParameterType::PARAMETER_DOUBLE) {
    RCLCPP_WARN(rclcpp::get_logger(name_), "%s is not double", name.c_str());
    return false;
  }
  ...
  if (name == "kp") {
    ...
    RCLCPP_WARN(rclcpp::get_logger(name_), "%s does not exist", name.c_str());
    return false;
  }
...
double PidController::integral_error() { return integral_error_; }
...
double PidController::update(double new_error, double actual_dt) {
  ...
  integral_error_ =
      std::clamp(integral_error_, coefficients_.min_i, coefficients_.max_i);
