...
casadi::Function align_yaw_function(const casadi_int &n) {
  ...
  return casadi::Function("align_yaw", {yaw_1, yaw_2}, {yaw_1_aligned},
                          {"yaw_1", "yaw_2"}, {"yaw_1_aligned"})
      .map(n);
}
...
casadi::Function align_abscissa_function(const casadi_int &n) {
  ...
  return casadi::Function("align_abscissa",
                          {abscissa_1, abscissa_2, total_distance},
                          {abscissa_1_aligned},
                          {"abscissa_1", "abscissa_2", "total_distance"},
                          {"abscissa_1_aligned"})
      .map(n);
...
casadi::Function c2d_function(const casadi_int &nx, const casadi_int &nu,
                              const double &dt) {
  using casadi::MX;
  using casadi::Slice;
...
casadi::Function rk4_function(const casadi_int &nx, const casadi_int &nu,
                              const double &dt,
  ...
  const auto out2 =
      dynamics(casadi::SXDict{{"x", x + dt / 2.0 * k1}, {"u", u}, {"k", k}});
  const auto k2 = out2.at("x_dot");
  const auto out3 =
      dynamics(casadi::SXDict{{"x", x + dt / 2.0 * k2}, {"u", u}, {"k", k}});
  const auto k3 = out3.at("x_dot");
  const auto out4 =
      dynamics(casadi::SXDict{{"x", x + dt * k3}, {"u", u}, {"k", k}});
...
casadi::Function rk4_function(const casadi_int &nx, const casadi_int &nu,
  ...
  const auto k4 = out4.at("x_dot");
  const auto out = x + dt / 6 * (k1 + 2 * k2 + 2 * k3 + k4);
  return casadi::Function("rk4", {x, u, k, dt}, {out}, {"x", "u", "k", "dt"},
                          {"xip1"});
}

casadi::Function euler_function(const casadi_int &nx, const casadi_int &nu,
  ...
  const auto x_dot =
      dynamics(casadi::SXDict{{"x", x}, {"u", u}, {"k", k}}).at("x_dot");
