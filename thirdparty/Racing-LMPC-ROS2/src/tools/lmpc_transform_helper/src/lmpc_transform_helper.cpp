...
namespace lmpc {
namespace utils {
TransformHelper::TransformHelper(rclcpp::Node &node) : node_(node) {
...
void TransformHelper::send_transform(
    const geometry_msgs::msg::TransformStamped &msg) {
  tf_broadcaster_->sendTransform(msg);
}

template <>
double TransformHelper::calc_yaw_difference(const double &yaw_1,
                                            const double &yaw_2) {
...
double TransformHelper::heading_from_quaternion(
