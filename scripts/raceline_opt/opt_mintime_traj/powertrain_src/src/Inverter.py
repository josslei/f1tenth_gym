...
class InverterModel:
    ...
    def initialize(self):
        ...
        self.temp_inv_n = ca.SX.sym("temp_inv_n")  # pyright: ignore[reportArgumentType]
    ...
    def get_increment(self, sf: ca.SX, temp_cool_mi: ca.SX, temp_cool_12: ca.SX):
        """
        Python version: 3.5
        Created by: Thomas Herrmann
        Created on: 01.04.2020
        ...
        self.dtemp = sf * (
            (
                ...
                - (self.temp_inv - (temp_cool_mi + temp_cool_12) / 2) / self.r_inv
            )
            / (self.pars["C_therm_inv"])
        )

    def get_loss(self, i_eff: ca.SX, v_dc: ca.SX, p_out_inv: ca.SX = None):  # pyright: ignore[reportArgumentType]
        """
        Python version: 3.5
        Created by: Thomas Herrmann
        Created on: 01.04.2020
        ...
        if self.pars["simple_loss"]:
            ...
            p_loss_cond = (
                3
                * i_eff
                * (self.pars["V_ce_offset"] + (self.pars["V_ce_slope"] * i_eff))
            )
    ...
    def ini_nlp_state(self, x: ca.SX, u: ca.SX):
        ...
        if self.pars["simple_loss"]:
            self.f_nlp = ca.Function(
                "f_nlp",
                [x, u],
                [self.p_loss_total, self.p_in_inv],
                ["x", "u"],
                ["p_loss_total", "p_inv_in"],
            )
        else:
            self.f_nlp = ca.Function(
                "f_nlp",
                [x, u],
                [self.p_loss_total, self.p_loss_switch, self.p_loss_cond],
                ["x", "u"],
                ["p_loss_total", "p_loss_switch", "p_loss_cond"],
            )
    ...
    def extract_sol(self, w: ca.SX, sol_states: ca.DM):
        ...
        if self.pars["simple_loss"]:
            ...
            self.p_in_inv = p_losses_opt[1::2]  # pyright: ignore[reportOptionalSubscript]
            ...
            self.p_loss_total = p_losses_opt[0::3]  # pyright: ignore[reportOptionalSubscript]
            self.p_loss_switch = p_losses_opt[1::3]  # pyright: ignore[reportOptionalSubscript]
            self.p_loss_cond = p_losses_opt[2::3]  # pyright: ignore[reportOptionalSubscript]
