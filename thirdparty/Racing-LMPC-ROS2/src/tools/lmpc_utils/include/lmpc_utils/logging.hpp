...
namespace lmpc {
namespace utils {
enum LogLevel : uint8_t { DEBUG = 0, INFO = 1, WARN = 2, ERROR = 3, FATAL = 4 };

class Logger {
public:
  typedef std::function<void(const LogLevel &, const std::string &)>
      LoggerCallback;
...
public:
   ...
   *
   * @param callback a callback function taking a LogLevel and a std::string
   * message.
   * @param min_level minimum log level to trigger this callback. Default to
   * debug.
   *
   ...
   */
  void register_callback(const LoggerCallbackKey &name,
  ...
  /**
   * @brief use this helper function to get a logger callback that dumps to
   * RCLCPP.
   *
