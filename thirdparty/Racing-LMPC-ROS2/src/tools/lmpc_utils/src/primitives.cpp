...
std::ostream &operator<<(std::ostream &os, const Pose2D &pose) {
  os << "Pose2D(" << pose.position.x << ", " << pose.position.y << ", "
     << pose.yaw << ")";
  return os;
}
...
std::ostream &operator<<(std::ostream &os, const FrenetPose2D &pose) {
  os << "FrenetPose2D(" << pose.position.s << ", " << pose.position.t << ", "
     << pose.yaw << ")";
  return os;
}

double lateral_sign(const Position2D &position, const Pose2D &p0) {
  return std::copysign(1.0,
                       std::cos(p0.yaw) * (position.y - p0.position.y) -
                           std::sin(p0.yaw) * (position.x - p0.position.x));
}
...
SpatialVelocity2D transform_velocity(const BodyVelocity2D &vb,
                                     const double &yaw) {
  return SpatialVelocity2D{vb.x * std::cos(yaw) - vb.y * std::sin(yaw),
                           vb.x * std::sin(yaw) + vb.y * std::cos(yaw),
                           vb.v_yaw};
}

BodyVelocity2D transform_velocity(const SpatialVelocity2D &vs,
                                  const double &yaw) {
  return BodyVelocity2D{vs.x * std::cos(yaw) + vs.y * std::sin(yaw),
                        -vs.x * std::sin(yaw) + vs.y * std::cos(yaw), vs.v_yaw};
}
} // namespace lmpc
