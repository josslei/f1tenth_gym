...
RacingSimulatorNode::RacingSimulatorNode(const rclcpp::NodeOptions &options)
    : rclcpp::Node("racing_simulator_node", options),
      ...
      sim_step_(0), lap_count_(0), tf_helper_(*this) {
  ...
  model_ = std::make_shared<SingleTrackPlanarModel>(base_model_config,
                                                    single_track_model_config);
  simulator_ = std::make_shared<RacingSimulator>(config_->dt, config_->x0,
                                                 track_, model_);
  ...
  cg_to_baselink_.setRotation(
      utils::TransformHelper::quaternion_from_heading(0.0));
  ...
  if (config_->publish_tf) {
    ...
    map_to_cg.setOrigin(
        tf2::Vector3(global_pose.position.x, global_pose.position.y, 0.0));
    map_to_cg.setRotation(
        utils::TransformHelper::quaternion_from_heading(global_pose.yaw));
  ...
  if (config_->visualize_vehicle) {
    ...
    const casadi::DM pts =
        casadi::DM::reshape(casadi::DM({lf, half_width, lf, -half_width, -lr,
                                        -half_width, -lr, half_width}),
                            2, 4);
  ...
  vehicle_state_pub_ =
      this->create_publisher<mpclab_msgs::msg::VehicleStateMsg>("vehicle_state",
                                                                1);

  // initialize visualization publishers
  if (config_->visualize_vehicle) {
    vehicle_polygon_pub_ =
        this->create_publisher<PolygonStamped>("vehicle_polygon", 1);
  }
  // initialize odom publishers
  vehicle_odom_pub_ =
      this->create_publisher<nav_msgs::msg::Odometry>("vehicle_odom", 1);

  // initialize subscribers
  vehicle_actuation_sub_ =
      this->create_subscription<mpclab_msgs::msg::VehicleActuationMsg>(
          "vehicle_actuation", 1,
          std::bind(&RacingSimulatorNode::on_actuation, this,
                    std::placeholders::_1));
  reset_state_sub_ =
      this->create_subscription<mpclab_msgs::msg::VehicleStateMsg>(
          "reset_state", 1,
          std::bind(&RacingSimulatorNode::on_reset_state, this,
                    std::placeholders::_1));
  ...
  if (config_->repeat_state_dt > 0) {
    state_repub_timer_ = this->create_wall_timer(
        std::chrono::milliseconds(
            static_cast<int>(config_->repeat_state_dt * 1000.0)),
...
void RacingSimulatorNode::on_actuation(
  ...
  // std::cout << "Simulation Step Time: " << duration.count() << "us" <<
  // std::endl;
}

void RacingSimulatorNode::on_reset_state(
    const mpclab_msgs::msg::VehicleStateMsg::SharedPtr msg) {
  // reset the simulator
  casadi::DM x;
  if (model_->get_base_config().modeling_config->use_frenet) {
    x = casadi::DM{msg->p.s,      msg->p.x_tran, msg->p.e_psi,
                   msg->v.v_long, msg->v.v_tran, msg->w.w_psi};
  } else {
    x = casadi::DM{msg->x.x,      msg->x.y,      msg->e.psi,
                   msg->v.v_long, msg->v.v_tran, msg->w.w_psi};
  }
...
void RacingSimulatorNode::on_state_repub_timer() {
  RCLCPP_INFO(
      this->get_logger(),
      "No actuation message received for %f seconds, republishing the state.",
...
void RacingSimulatorNode::update_vehicle_state_msg(
  ...
  const auto k = static_cast<double>(track_->curvature_interpolation_function()(
      casadi::DM(frenet_pose.position.s))[0]);
  ...
  vehicle_state_msg_->lap_num =
      lap_count_ + frenet_pose.position.s / track_->total_length();
}
...
void RacingSimulatorNode::on_state_update() {
  ...
  if (vehicle_state_msg_->p.s - frenet_pose.position.s >
      0.5 * track_->total_length()) {
    lap_count_++;
  }
...
RCLCPP_COMPONENTS_REGISTER_NODE(
    lmpc::simulation::racing_simulator::RacingSimulatorNode)
