...
size_t SingleTrackPlanarModel::nx() const { return 6; }
...
void SingleTrackPlanarModel::add_nlp_constraints(casadi::Opti &opti,
                                                 const casadi::MXDict &in) {
  const auto &u = in.at("u");
  casadi::MX fd, fb, delta;
  if (config_->simplify_lon_control) {
    fd = u(UIndexSimple::LON) *
         (casadi::MX::tanh(u(UIndexSimple::LON)) * 0.5 + 0.5) * 1000.0;
    fb = u(UIndexSimple::LON) *
         (casadi::MX::tanh(-u(UIndexSimple::LON)) * 0.5 + 0.5) * 1000.0;
  ...
  const auto &max_steer_rate = get_base_config().steer_config->max_steer_rate;
  ...
  if (in.count("x")) {
    ...
    //     lmpc::utils::align_yaw<casadi::MX>(xip1_temp(XIndex::YAW),
    //     x(XIndex::YAW));
    // }
    ...
    if (config_->simplify_lon_control) {
      opti.subject_to(
          opti.bounded(Fb_max / 1000.0, u(UIndexSimple::LON), Fd_max / 1000.0));
    } else {
  ...
  if (in.count("uip1")) {
    ...
    if (config_->simplify_lon_control) {
      opti.subject_to(
          opti.bounded(Fb_max / 1000.0 / Tb,
                       (uip1(UIndexSimple::LON) - u(UIndexSimple::LON)) / t,
                       Fd_max / 1000.0 / Td));
      opti.subject_to(opti.bounded(
          -max_steer_rate, (uip1(UIndexSimple::STEER_SIMPLE) - delta) / t,
          max_steer_rate));
    } else {
  ...
  if (in.count("dui")) {
    ...
    if (config_->simplify_lon_control) {
      opti.subject_to(opti.bounded(Fb_max / 1000.0 / Tb, dui(UIndexSimple::LON),
                                   Fd_max / 1000.0 / Td));
      opti.subject_to(opti.bounded(
          -max_steer_rate, dui(UIndexSimple::STEER_SIMPLE), max_steer_rate));
    } else {
...
void SingleTrackPlanarModel::calc_lon_control(const casadi::DMDict &in,
                                              double &throttle,
...
void SingleTrackPlanarModel::calc_lat_control(const casadi::DMDict &in,
...
void SingleTrackPlanarModel::compile_dynamics() {
  ...
  // const auto & twf = get_base_config().chassis_config->tw_f;  // front track
  // width const auto & twr = get_base_config().chassis_config->tw_r;  // rear
  // track width
  const auto &fr =
  ...
  // const auto & eps_f = tyre_f.pacejka_eps;  // extended magic formula epsilon
  // - front
  ...
  // const auto & eps_r = tyre_r.pacejka_eps;  // extended magic formula epsilon
  // - rear
  ...
  const auto Fz_r = 0.5 * m * GRAVITY * lf / (lf + lr) +
                    0.5 * hcog / (lf + lr) * m * ax +
  ...
  const auto a_fl = delta - atan((lf * omega + vy) / (vx + 1e-3));
  ...
  //   (Fy_rl + Fy_rr) * sin(beta) - (Fy_fl + Fy_fr) * sin(delta - beta) - 0.5 *
  //   cd * rho * A * v_sq * cos(beta));
  // const auto beta_dot = -omega + 1.0 / (m * v) *
  //   (-(Fx_rl + Fx_rr) * sin(beta) + (Fx_fl + Fx_fr) * sin(delta - beta) +
  //   (Fy_rl + Fy_rr) * cos(
  //     beta) + (Fy_fl + Fy_fr) * cos(delta - beta) + 0.5 * cd * rho * A * v_sq
  //     * sin(beta));
  const auto omega_dot =
      1.0 / Jzz *
      (-(2 * Fy_rl) * lr +
       ((2 * Fy_fl) * cos(delta) + (2 * Fx_fl) * sin(delta)) * lf);
  ...
  const auto x_dot =
      vertcat(px_dot, py_dot, phi_dot, vx_dot, vy_dot, omega_dot);
  const auto Fx_ij = vertcat(Fx_fl, Fx_rl);
  const auto Fy_ij = vertcat(Fy_fl, Fy_rl);
  const auto Fz_ij = vertcat(Fz_fl, Fz_rl);

  dynamics_ = casadi::Function("single_track_planar_model_dynamics", {x, u, k},
                               {x_dot, Fx_ij, Fy_ij, Fz_ij}, {"x", "u", "k"},
  ...
  dynamics_jacobian_ =
      casadi::Function("single_track_planar_model_dynamics_jacobian", {x, u, k},
                       {Ac, Bc}, {"x", "u", "k"}, {"A", "B"});
  ...
  discrete_dynamics_ = casadi::Function(
      "single_track_planar_model_discrete_dynamics", {x, u, k, dt},
  ...
  discrete_dynamics_jacobian_ = casadi::Function(
      "single_track_planar_model_discrete_dynamics_jacobian", {x, u, k, dt},
      {Ad, Bd, gd}, {"x", "u", "k", "dt"}, {"A", "B", "g"});
  ...
  if (config_->simplify_lon_control) {
    ...
    const auto u_base_out =
        SX::vertcat({u_derived_sym(UIndexSimple::LON) * 1 /
                         (1 + SX::exp(-u_derived_sym(UIndexSimple::LON))),
                     u_derived_sym(UIndexSimple::LON) * 1 /
                         (1 + SX::exp(u_derived_sym(UIndexSimple::LON))),
                     u_derived_sym(UIndexSimple::STEER_SIMPLE)});
    const auto u_derived_out = SX::vertcat(
        {SX::if_else(abs(u_base_sym(UIndex::FD)) > abs(u_base_sym(UIndex::FB)),
                     u_base_sym(UIndex::FD), u_base_sym(UIndex::FB)),
         u_base_sym(UIndex::STEER)});

    to_base_control_ =
        casadi::Function("to_base_control", {x_sym, u_derived_sym},
                         {u_base_out}, {"x", "u"}, {"u_out"});
    from_base_control_ =
        casadi::Function("from_base_control", {x_sym, u_base_sym},
                         {u_derived_out}, {"x", "u"}, {"u_out"});
    to_base_state_ = casadi::Function("to_base_state", {x_sym, u_derived_sym},
                                      {x_sym}, {"x", "u"}, {"x_out"});
    from_base_state_ = casadi::Function("from_base_state", {x_sym, u_base_sym},
                                        {x_sym}, {"x", "u"}, {"x_out"});
  }
}
} // namespace single_track_planar_model
