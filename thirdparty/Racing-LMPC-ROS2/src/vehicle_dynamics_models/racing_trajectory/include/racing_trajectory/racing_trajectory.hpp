...
public:
   ...
   * @param initialize_with_previous if true, the previous frenet pose supplied
   * here will be used as initial guess.
   */
  void global_to_frenet(const Pose2D &global_pose, FrenetPose2D &frenet_pose,
   ...
   * Takes a vector {x, y, theta, s0, t0} as input and returns a vector {s, t,
   * xi}. Less useful than the frenet_to_global_function() because it requires
   * the initial guesses s0 and t0. This function is not differentiable since it
   * uses a qp solver.
   *
...
protected:
  ...
  casadi::Function
      norm_2_; // helper function to compute the norm of all waypoints
