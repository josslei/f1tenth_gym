...
VanillaControllerNode::VanillaControllerNode(const rclcpp::NodeOptions &options)
    : rclcpp::Node("vanilla_controller_node", options),
      ...
      model_(vehicle_model::vehicle_model_factory::load_vehicle_model(
          utils::declare_parameter<std::string>(
              this, "vanilla_controller_node.vehicle_model_name"),
          this)),
      controller_(
          std::make_shared<VanillaController>(config_, model_, track_)) {
  ...
  vehicle_state_sub_ =
      this->create_subscription<mpclab_msgs::msg::VehicleStateMsg>(
          "vehicle_state", 1,
          std::bind(&VanillaControllerNode::on_new_state, this,
                    std::placeholders::_1));
  ...
  if (config_->step_mode == VanillaControllerStepMode::CONTINUOUS) {
    ...
    step_timer_ = this->create_wall_timer(
        ...
        std::bind(&VanillaControllerNode::on_step_timer, this));
  }
}

void VanillaControllerNode::on_new_state(
...
void VanillaControllerNode::on_step_timer() {
  ...
  const auto vel_ref =
      track_->velocity_interpolation_function()(x_ic(XIndex::PX))[0];
...
int main(int argc, char *argv[]) {
  ...
  auto node =
      std::make_shared<lmpc::mpc::vanilla_controller::VanillaControllerNode>(
          options);
