...
base_vehicle_model::BaseVehicleModel::SharedPtr
load_vehicle_model(const std::string model_name, rclcpp::Node *node) {
  const auto base_config =
      lmpc::vehicle_model::base_vehicle_model::load_parameters(node);
  if (model_name == "kinematic_bicycle_model") {
    const auto config =
        lmpc::vehicle_model::kinematic_bicycle_model::load_parameters(node);
    return std::make_shared<kinematic_bicycle_model::KinematicBicycleModel>(
        base_config, config);
  } else if (model_name == "single_track_planar_model") {
    const auto config =
        lmpc::vehicle_model::single_track_planar_model::load_parameters(node);
    return std::make_shared<single_track_planar_model::SingleTrackPlanarModel>(
        base_config, config);
  } else if (model_name == "double_track_planar_model") {
    const auto config =
        lmpc::vehicle_model::double_track_planar_model::load_parameters(node);
    return std::make_shared<double_track_planar_model::DoubleTrackPlanarModel>(
        base_config, config);
  } else {
    RCLCPP_FATAL(node->get_logger(), "Vehicle model %s cannot be found.",
                 model_name.c_str());
    return nullptr;
  }
}
