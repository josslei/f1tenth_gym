...
from opt_mintime_traj.src.friction_map_interface import FrictionMapInterface


def extract_friction_coeffs(
    reftrack: np.ndarray,
    ...
    assert tpadata_path is not None
    map_interface = FrictionMapInterface(
        tpamap_path=tpamap_path, tpadata_path=tpadata_path
    )
    ...
    for i in range(num_steps):
        ...
        for j in range(num_right + num_left + 1):
            xy[0:2, j] = (refline_cl[i, :] + wb_f * tang_vec[i, :]) - (
                n_pos[j] + 0.5 * width
            ) * normvectors_cl[i, :]
            xy[2:4, j] = (refline_cl[i, :] + wb_f * tang_vec[i, :]) - (
                n_pos[j] - 0.5 * width
            ) * normvectors_cl[i, :]
            xy[4:6, j] = (refline_cl[i, :] - wb_r * tang_vec[i, :]) - (
                n_pos[j] + 0.5 * width
            ) * normvectors_cl[i, :]
            xy[6:8, j] = (refline_cl[i, :] - wb_r * tang_vec[i, :]) - (
                n_pos[j] - 0.5 * width
            ) * normvectors_cl[i, :]
        ...
        if print_debug:
            tph.progressbar.progressbar(  # pyright: ignore[reportAttributeAccessIssue]
                i, num_steps, "Extraction of Friction Coefficients from Friction Map"
            )
